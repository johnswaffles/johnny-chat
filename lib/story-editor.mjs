...
// Reasoning and visible prose share this allowance. Never accept partial JSON.
export async function withStoryOutputBudget(generate, requested = 25000) {
  let budget = Math.min(64000, Math.max(25000, requested));
  for (;;) {
    try { return await generate(budget); }
    catch (error) {
      if (error?.code !== 'max_output_tokens') throw error;
      if (budget >= 64000) {
        const stopped = new Error('This section needs more output space than the automatic retry allows. Completed sections are saved. Resume the unfinished edit to try again.');
        stopped.code = 'max_output_tokens';
        throw stopped;
      }
      budget = Math.min(64000, budget * 2);
    }
  }
}
