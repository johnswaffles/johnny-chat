import {grantFinalBenediction,updateFinalBenediction} from './final-benediction.js?v=20260923-teamdock1';
import {wizardAttackRange,reckoningDamage} from './eventide-ascendancy.js?v=20260923-teamdock1';
import {prepareWizardPosition} from './wizard-positioning.js?v=20260923-teamdock1';
import {stormwardDamage} from './storm-dragon.js?v=20260923-teamdock1';
import {castWizardSpell,updateWizardMagic,wizardIncomingDamage} from './wizard-magic.js?v=20260923-teamdock1';
import {cancelSidePull,prepareTankPull,holdForTankPull,markFrontFallback,bearRearPosition,bearOrbitStep,combatRole,isTeamHealer,eligibleMember,TEAM_RULES,teams,assignTeam,leaveTeam,selectTeam,tankTarget,claimThreat,updateTeams,prepareTeamAttack,updateTeamApproaches,teamMovePoint} from './combat-teams.js?v=20260923-teamdock1';
import { BEAR_VARIANT_IDS } from './bear-variants.js?v=20260923-teamdock1';
import { kingsbaneMultiplier, updateGreatwoodDefiance, bearEnrageActive, combatRadius, inBearSwipe, BEAR_FURY, FIGHTER_PURSUIT, isFighter, bearFuryActive, bearArrowDamage, bearIncomingDamage, corpseLifetime } from './bear-combat.js?v=20260923-teamdock1';
import {updateLastBastion,lastBastionDamage,isCurseImmune,isWardProtected,strikeDamage} from './unit-status.js?v=20260923-teamdock1';
import { initialWildlifeState, updateWildlife } from './wildlife.js?v=20260923-teamdock1';
import { GRIZZLY_PURSUIT, grizzlyAttackDefinition, updateGrizzlyMotion } from './grizzly-motion.js?v=20260923-teamdock1';
import { assignEnemyEconomy, assignEnemyPatrols } from './enemy-routines.js?v=20260923-teamdock1';
import { landscapeHash, landscapeNoise, woodlandDensity, woodlandRidgeZ, FOREST_LIMITS } from './landscape-layout.js?v=20260923-teamdock1';
import { BUILDING_ART_VERSION } from './building-depth-data.js?v=20260923-teamdock1';
import { readSavedGameForBuildingUpgrade } from './building-save-backup.js?v=20260923-teamdock1';
import { hasBuildingOutline, buildingActorProfile, outlineBounds, outlineApproaches, distanceToOutline, withinOutlineDistance, projectOutsideOutline, cellIntersectsOutline, translatedOutline, polygonsOverlap } from './building-geometry.js?v=20260923-teamdock1';
import { BUILDING_TYPES, CONFIG, ENEMY_AI, FACTION, FIRST_AGE_BUILD_BLUEPRINTS, FIRST_AGE_MILESTONES, FIRST_AGE_TECHNOLOGIES, FIRST_AGE_WORK_PRIORITIES, INITIAL_RESOURCES, PRODUCTION_TYPES, RESOURCE_SIZE_TIERS, RESOURCE_TYPES, SPACING_ROLES, UNIT_TYPES, resourceDepletionStage } from './config.js?v=20260923-teamdock1';
import { findPath } from './pathfinding.js?v=20260923-teamdock1';
import { ResourceConnectivity } from './resource-connectivity.js?v=20260923-teamdock1';
import { ANIMATION_EVENT_TIMINGS, ANIMATION_EVENTS, CrownforgeAnimationSystem } from './animation.js?v=20260923-teamdock1';
...
// Units should interact with a structure from its illustrated edge, not from
// the transparent pixels inside the raster. Existing landmarks may author a
// larger ring; future solid buildings get this safe default automatically.
const DEFAULT_BUILDING_UNIT_EXCLUSION_PADDING = 0.38;
const PRODUCTION_SPAWN_MARGIN = 2.2;
...
// Corpse lifetime is shared with rendering; bears complete a painted collapse before fading.
...
function buildingUnitExclusionPadding(buildingOrType) {
  const type = typeof buildingOrType === 'string' ? buildingOrType : buildingOrType?.type;
  const blueprint = BUILDING_TYPES[type];
  if (!blueprint || blueprint.walkable || blueprint.wall || blueprint.gate || blueprint.road) {
    return blueprint?.unitExclusionPadding ?? 0;
  }
  return blueprint.unitExclusionPadding ?? DEFAULT_BUILDING_UNIT_EXCLUSION_PADDING;
}
...
function setUnitFacing(unit, dx, dz, force = false) {
  ...
  if (['villager','ashenForager','wizard','militia'].includes(unit.type)) {
...
export class CrownforgeSimulation {
  ...
  reset() {
    ...
    for (const building of this.buildings) this._relocateUnitsFromBuilding(building);
  ...
  _seedWorld(worldSeed = this.activeWorldSeed ?? this.worldSeed) {
    ...
    this.addResource('berry', 'food', 43, 102, 105, 1, { sizeTier: 'small' });
    this.addResource('berry', 'food', 82, 41, 105, 0, { sizeTier: 'small' });
    this.addResource('stone', 'stone', 109, 99, 360, 1, { sizeTier: 'medium' });
    this.addResource('stone', 'stone', 111, 116, 900, 0, { sizeTier: 'large' });
  ...
  _updateFixed(dt) {
    ...
    if (this.phase !== 'playing') {
      for (const unit of this.units) {
        ...
        if (unit.dead && unit.deathAge >= corpseLifetime(unit) && unit.lastAnimationEvent?.name !== ANIMATION_EVENTS.deathComplete) {
          this.animation.emit(unit, ANIMATION_EVENTS.deathComplete);
        }
      }
      this.units = this.units.filter((unit) => !unit.dead || unit.deathAge < corpseLifetime(unit));
      return;
    }
    this.clock += dt;
  ...
  _updateDemolition(building, dt) {
    ...
    for (const worker of workers) {
      if (UNIT_TYPES[worker.type]?.worker) worker.workCyclePhase = (building.demolitionTimer / DEMOLITION_STRIKE_INTERVAL + .6) % 1;
    }
  ...
  _updateConstruction(building, dt) {
    ...
    for (const builder of builders) {
      if (UNIT_TYPES[builder.type]?.worker) builder.workCyclePhase = ((building.constructionTimer ?? 0) / visualStrikeInterval + .6) % 1;
    }
    const constructionComplete = building.progress >= 1 && building.hp >= building.maxHp - BUILDING_REPAIR_EPSILON;
    if (constructionComplete) {
      if (!repairing) this.navigationVersion += 1;
      building.progress = 1;
  ...
  _updateField(building, dt) {
    ...
    if (UNIT_TYPES[farmer.type]?.worker) farmer.workCyclePhase = (building.fieldTimer / 1.6 + .6) % 1;
  ...
  _findUnitSpawnPoint(building, type, queueDepth = 0) {
    ...
    const spawnMargin = Math.max(PRODUCTION_SPAWN_MARGIN, BUILDING_TYPES[building.type]?.spawnDistance ?? 0);
    const candidates = this._buildingApproachPoints(building, spawnMargin + wobble, { type });
  ...
  _updateUnit(unit, dt) {
    updateGreatwoodDefiance(unit,this.units,dt);
    updateLastBastion(unit,dt);
    const grizzlyStart = unit.type === 'grizzly' ? { x:unit.x, z:unit.z } : null;
    const fighterStart = isFighter(unit) ? {x:unit.x,z:unit.z} : null;
    if (unit.dead) {
      unit.deathAge += dt;
      unit.animClock += dt;
      unit.motionSpeed = 0;
    ...
    if (unit.command === 'move' || unit.command === 'gather' || unit.command === 'return' || unit.command === 'attack' || unit.command === 'build' || unit.command === 'demolish' || unit.command === 'field') {
      ...
      else if (fighterStart && unit.command === 'attack') this._prepareFighterPursuit(unit);
    ...
    if(fighterStart)unit.combatLocomotionTime=(unit.combatLocomotionTime??0)+distance(unit,fighterStart)/Math.max(UNIT_TYPES[unit.type].speed,.01);
  ...
  _followPath(unit, dt) {
    ...
    if (next) {
      ...
      if (length < (unit.grizzlyPursuitDirect || unit.fighterMovingAttack ? .002 : .12)) {
        ...
        unit.z = next.z;
        unit.path.shift();
        // A consumed corner is a real stopping constraint. Residual mounted
        // momentum at accelerated speed could drift past it and repeatedly
        // invalidate the next leg beside the enlarged painted outline.
        unit.velocityX = 0;
        unit.velocityZ = 0;
      } else {
    ...
    const velocityDeltaX = desiredX - unit.velocityX;
    const velocityDeltaZ = desiredZ - unit.velocityZ;
    const velocityDelta = Math.hypot(velocityDeltaX, velocityDeltaZ);
    const response = Math.min(1, acceleration * dt / Math.max(velocityDelta, 0.000001));
    // Accelerate along the requested vector. Independent component clamps
    // turn every departure from rest into a 45-degree step, which can leave
    // a safe tangent route and clip an irregular building corner.
    unit.velocityX += velocityDeltaX * response;
    unit.velocityZ += velocityDeltaZ * response;
  ...
  _beginReturn(unit) {
    ...
    this._releaseUnitFromBuildingPerimeter(unit);
    this._releaseResourceSlot(unit);
    this._releaseStorageSlot(unit);
  ...
  _buildingApproachPoints(building, margin = BUILDING_INTERACTION_DISTANCE, unit = null) {
    if (hasBuildingOutline(building)) return outlineApproaches(building, margin, unit)
      .slice(0, this._buildingInteractionSlotCount(building))
      .map(point => ({ ...point, x: clamp(point.x, 0.55, CONFIG.mapWidth - 0.55), z: clamp(point.z, 0.55, CONFIG.mapHeight - 0.55) }));
    const blueprint = BUILDING_TYPES[building.type];
    ...
    const unitClearance = visualClearance + unitExclusion;
    ...
    if (blueprint.wall) {
      ...
      if (geometry) {
        ...
        const sideDistance = geometry.halfThickness + unitExclusion + margin;
        ...
        const endDistance = geometry.halfLength + margin + unitExclusion;
  ...
  _distanceToBuildingEdge(point, building) {
    if (hasBuildingOutline(building)) return distanceToOutline(point, building);
  ...
  _distanceToBuildingUnitEdge(point, building, unit = point) {
    if (hasBuildingOutline(building)) return distanceToOutline(point, building, buildingActorProfile(unit));
    const extraClearance = buildingUnitExclusionPadding(building);
    const wallGeometry = this._wallLineGeometry(building);
    if (wallGeometry) {
      return Math.max(0, this._distanceToWallCenterline(point, building) - wallGeometry.halfThickness - extraClearance,
        this._wallGateClearance(point, building).clearance);
    }
  ...
  _wallGateClearance(point, wall) {
    if (this.gatePassageVersion !== this.navigationVersion) {
      this.gatePassageVersion = this.navigationVersion;
      this.gatePassages = this.buildings.filter(b => BUILDING_TYPES[b.type]?.gate && !b.destroyed && b.progress >= 1);
    }
    const geometry = this._wallLineGeometry(wall);
    let best = { clearance: -Infinity, gate: null, along: 0 };
    if (!geometry) return best;
    for (const gate of this.gatePassages) {
      const direction = wallDirectionFromOptions(gate);
      if (Math.abs(direction.x * geometry.direction.x + direction.z * geometry.direction.z) < .99) continue;
      const dx = gate.x - geometry.center.x, dz = gate.z - geometry.center.z;
      if (Math.abs(dx * geometry.direction.z - dz * geometry.direction.x) > .2
        || Math.abs(dx * geometry.direction.x + dz * geometry.direction.z) > geometry.halfLength + 2) continue;
      const along = (point.x - gate.x) * direction.x + (point.z - gate.z) * direction.z;
      const clearance = BUILDING_TYPES[gate.type].footprint.width / 2 - Math.abs(along);
      if (clearance > best.clearance) best = { clearance, gate, along, direction };
    }
    return best;
  }
  ...
  _sendUnitToCrownHallStairs(unit, building, index = 0, total = 1) {
    const target = hasBuildingOutline(building)
      ? this._buildingApproachPoints(building, BUILDING_INTERACTION_DISTANCE, unit)[index % 3]
      : this._crownHallStairTarget(building, index, total);
    if (!target) return false;
    unit.stairAccess = !hasBuildingOutline(building);
    ...
    if (!path) {
      ...
      unit.command = 'idle';
      unit.visualState = 'idle';
      unit.actionLabel = 'Crown Hall approach blocked';
      return false;
    }
    unit.path = path;
    ...
    unit.command = 'move';
    unit.visualState = 'walk';
    unit.actionLabel = 'Walking to Crown Hall';
    this._resetMovementTracking(unit);
    return true;
  }

  _buildingEntityBounds(building, padding = 0, profile = 'material') {
    if (hasBuildingOutline(building)) return outlineBounds(building, padding, profile);
    const footprint = this._buildingFootprint(building);
    const center = this._buildingCollisionCenter(building);
    const visualClearance = BUILDING_TYPES[building.type].collisionClearance ?? 0;
    return {
  ...
  _placementEvictionCandidates(unit, building) {
    ...
    for (const margin of [0.82, 1.45, 2.2, 3.1, 4.2]) {
      candidates.push(...this._buildingApproachPoints(building, margin, unit));
    }
    const center = this._buildingCollisionCenter(building);
    const footprint = this._buildingFootprint(building);
    const unitClearance = (BUILDING_TYPES[building.type]?.collisionClearance ?? 0)
      + buildingUnitExclusionPadding(building);
  ...
  _relocateUnitsFromBuilding(building) {
    ...
    for (const unit of affected) {
      ...
      unit.command = 'idle';
      unit.visualState = unit.carryType ? `carry:${unit.carryType}` : 'idle';
      unit.actionLabel = 'Clear of the building';
      unit.pathBlocked = false;
      unit.recoveryAvailable = false;
      unit.stuckTimer = 0;
  ...
  _releaseUnitFromBuildingPerimeter(unit) {
    const unitPoint = { x: unit.x, z: unit.z };
    if (!this._pointBlockedForUnit(unit, unitPoint)) return false;
    const unitPadding = (UNIT_TYPES[unit.type]?.radius ?? 0.4) + UNIT_STATIC_CLEARANCE;
    const buildingBlockers = this._staticBlockerCandidates(unit, STATIC_BLOCKER_QUERY_RADIUS + 2)
      .filter((building) => building.kind === 'building'
        && this._buildingHasCollision(building)
        && this._distanceToBuildingUnitEdge(unitPoint, building, unit) < unitPadding)
      .sort((a, b) => this._distanceToBuildingUnitEdge(unitPoint, a) - this._distanceToBuildingUnitEdge(unitPoint, b));
    if (!buildingBlockers.length) return false;

    // A unit can only arrive here from a pre-existing save, a building that
    // finished around it, or a deliberately tight test layout. Do not make
    // it walk through its own safety ring. Relocate it to a clear exterior
    // station, then let the ordinary pathfinder calculate the next order.
    for (const building of buildingBlockers) {
      const candidates = [
        ...this._placementEvictionCandidates(unit, building),
        ...this._buildingApproachPoints(building, 1.35),
        ...this._buildingApproachPoints(building, 2.4),
        ...this._buildingApproachPoints(building, 3.6),
      ];
      const point = candidates
        .filter((candidate) => !this._pointBlockedForUnit(unit, candidate))
        .sort((a, b) => distance(unitPoint, a) - distance(unitPoint, b))[0];
      if (!point) continue;
      unit.x = point.x;
      unit.z = point.z;
      unit.path = [];
  ...
  _isPathCellBlocked(unit, cellX, cellZ, placement = null, allowedPoint = null) {
    ...
    for (const building of candidates) {
      ...
      if (hasBuildingOutline(building)) {
        if (cellIntersectsOutline(cellX, cellZ, building, padding, buildingActorProfile(unit))) return true;
        continue;
      }
      if (BUILDING_TYPES[building.type]?.wall) {
        if (this._distanceToBuildingUnitEdge(cellPoint, building, unit) < padding) return true;
        continue;
      }
    ...
    if (placement && (hasBuildingOutline(placement)
      ? cellIntersectsOutline(cellX, cellZ, placement, padding, buildingActorProfile(unit))
      : (this._buildingBlocksCell(cellX, cellZ, placement) || this._cellIntersectsBuilding(cellX, cellZ, placement, padding)))) return true;
  ...
  _buildPath(unit, target, placement = null, { directOnly = false, allowStartConnector = true } = {}) {
    ...
    if (path.length && targetCellOpen && distance(path[path.length - 1], safeTarget) <= 1.3) {
      const prior = path.length > 1 ? path[path.length - 2] : unit;
      if (!this._pathSegmentBlocked(unit, prior, safeTarget, placement)) {
        path[path.length - 1] = { x: safeTarget.x, z: safeTarget.z };
      } else if (!this._pathSegmentBlocked(unit, path[path.length - 1], safeTarget, placement)) {
        // Keep the final safe corner before approaching an exact door/work
        // station. Replacing it can cut through a curved building outline.
        path.push({ x: safeTarget.x, z: safeTarget.z });
      } else {
        this._cachePath(cacheKey, null);
        return null;
      }
  ...
  _findStorageRoute(unit, resourceType = unit.carryType) {
    ...
    // Keep dedicated first-age depots authoritative for their resource. A
    // worker recovering from another building's perimeter should still use a
    // nearby Granary/Timber Yard/Stonewright Yard/Ore Wash when one exists,
    // instead of switching to the Crown Hall simply because the egress point
    // is temporarily closer to the civic center.
    const dedicated = storages.filter((building) => {
      const accepted = BUILDING_TYPES[building.type]?.acceptsResources;
      return Array.isArray(accepted) && accepted.length === 1 && accepted.includes(resourceType);
    });
    const candidates = dedicated.length ? dedicated : storages;
  ...
  _combatApproachPoints(unit, target) {
    ...
    if(unit.frontFallback?.targetId===target.id)points.sort((a,b)=>distance(unit,a.point)-distance(unit,b.point));
    return points;
  }
  ...
  _bestCombatRoute(unit, target) {
    ...
    if(target.kind==='unit'&&unit.teamId&&combatRole(unit)==='damage'&&!(unit.frontFallback?.targetId===target.id&&unit.frontFallback.until>this.clock&&distance(unit.frontFallback,target)<3)){markFrontFallback(this,unit,target);return this._bestCombatRoute(unit,target);}
    return null;
  }
  ...
  _retaliateGrizzly(bear,attacker) {
    ...
    if(bear.attackTarget===attacker.id && bear.command==='attack')return;
    this._releaseCombatSlot(bear);
    if(bear.command!=='attack')this._cancelAttackCycle(bear);
    bear.command='attack';bear.attackTarget=attacker.id;bear.attackTargetKind='unit';
    bear.path=[];bear.routeTarget=null;bear.attackRepathCooldown=0;
    this._reserveCombatSlot(bear,attacker,bear.id%COMBAT_SLOT_COUNT);
    bear.actionLabel='Turning on '+UNIT_TYPES[attacker.type].label;
    setUnitFacing(bear,attacker.x-bear.x,attacker.z-bear.z,true);
  }
  ...
  _applyGrizzlyCleave(bear,primary) {
    ...
    for(const target of this.units){
      if(target===bear||target.dead||target.hp<=0||target.faction===bear.faction
        ||isWardProtected(target)||distance(bear,target)>BEAR_FURY.radius
        ||!this._hasCombatLineOfSight(bear,target))continue;
      const result=this._applyUnitDamage(target,target.maxHp*.5,bear,{damageType:'special',areaOfEffect:true});
  ...
  _updateAttack(unit, dt) {
    ...
    if(!target && unit.attackPhase!=='approach' && unit.attackEventFired){
      const rule=UNIT_TYPES[unit.type],timing=unit.type==='grizzly'?grizzlyAttackDefinition(unit):(rule.attackTiming??{anticipation:.25,contact:.45,recovery:.3});
      const duration=timing.duration??rule.cooldown;
      unit.path=[];unit.velocityX=unit.velocityZ=0;
      unit.attackPhaseElapsed+=dt;
      if(unit.attackPhase==='contact' && unit.attackPhaseElapsed>=duration*timing.contact){unit.attackPhase='recovery';unit.attackPhaseElapsed=0;}
      else if(unit.attackPhase==='recovery' && unit.attackPhaseElapsed>=duration*timing.recovery)this._cancelAttackCycle(unit);
      return;
    }
    ...
    if((unit.type!=='grizzly' && !unit.fighterMovingAttack) || (unit.type==='grizzly' && unit.grizzlyAttackVariant==='rear')) {
      unit.path = [];unit.velocityX = 0;unit.velocityZ = 0;
    }
    unit.visualState = 'attack';
    unit.actionLabel = `${unit.grizzlyMovingAttack?'Chasing and swiping at':unit.fighterMovingAttack?'Running strike against':'Attacking'} ${this._targetLabel(target)}`;
    unit.attackTimer += dt;
    unit.attackPhaseElapsed += dt;
      ...
      if (!unit.attackEventFired && unit.attackPhaseElapsed >= contactEventTime) {
        ...
        if (validContact) {
          ...
          if (target.kind === 'building') {
            ...
            if(unit.type==='grizzly')this._applyGrizzlyCleave(unit,target);
  ...
  _killUnit(unit, killer) {
    ...
    for (const attacker of this.units) {
      ...
      if(attacker===killer && attacker.attackPhase!=='approach' && attacker.attackEventFired)continue;
  ...
  _cellIntersectsBuilding(cellX, cellZ, building, padding = 0) {
    if (hasBuildingOutline(building)) return cellIntersectsOutline(cellX, cellZ, building, padding);
    const wallGeometry = this._wallLineGeometry(building);
    if (wallGeometry) {
  ...
  _constrainUnitPosition(unit, previousX = unit.x, previousZ = unit.z) {
    ...
    for (const building of nearbyBlockers) {
      ...
      if (hasBuildingOutline(building)) {
        const exterior = projectOutsideOutline(unit, building, radius, buildingActorProfile(unit));
        if (exterior) {
          unit.x = exterior.x;
          unit.z = exterior.z;
          if (unit.velocityX * exterior.nx + unit.velocityZ * exterior.nz < 0) {
            unit.velocityX = 0;
            unit.velocityZ = 0;
          }
        }
      ...
      const wall = this._wallLineGeometry(building);
      if (wall) {
        const passage = this._wallGateClearance(unit, building);
        if (passage.clearance >= radius) continue;
        const along = clamp((unit.x - wall.center.x) * wall.direction.x + (unit.z - wall.center.z) * wall.direction.z,
          -wall.halfLength, wall.halfLength);
        const closest = { x: wall.center.x + wall.direction.x * along, z: wall.center.z + wall.direction.z * along };
        let dx = unit.x - closest.x, dz = unit.z - closest.z;
        const distance = Math.hypot(dx, dz), clearance = wall.halfThickness + unitExclusion + radius;
        if (distance >= clearance) continue;
        if (passage.clearance > 0) {
          const move = (radius - passage.clearance + .015) * -Math.sign(passage.along || 1);
          unit.x += passage.direction.x * move; unit.z += passage.direction.z * move;
        } else {
          if (distance < .00001) { dx = -wall.direction.z; dz = wall.direction.x; }
          const length = Math.hypot(dx, dz);
          unit.x = closest.x + dx / length * (clearance + .015);
          unit.z = closest.z + dz / length * (clearance + .015);
        }
        continue;
      }
  ...
  _placementPolygon(building) {
    if (hasBuildingOutline(building)) return translatedOutline(building);
    const wall = this._wallLineGeometry(building);
    if (wall) {
      const length = wall.halfLength + wall.halfThickness;
      return [[-1,-1],[1,-1],[1,1],[-1,1]].map(([along, across]) => [
        wall.center.x + wall.direction.x * length * along - wall.direction.z * wall.halfThickness * across,
        wall.center.z + wall.direction.z * length * along + wall.direction.x * wall.halfThickness * across,
      ]);
    }
    const b = this._buildingEntityBounds(building);
    return [[b.minX,b.minZ],[b.maxX,b.minZ],[b.maxX,b.maxZ],[b.minX,b.maxZ]];
  }
  ...
  getPlacementCheck(type, point, options = {}) {
    ...
    if (this.buildings.some((building) => {
      ...
      if (hasBuildingOutline(placement) || hasBuildingOutline(building)) {
        return polygonsOverlap(this._placementPolygon(placement), this._placementPolygon(building), placementPadding);
      }
      return true;
    })) {
    ...
    if (this.resourcesNodes.some((node) => node.amount > 0
      ...
      && intersectsFoundation(node, resourceFootprint(node)))) {
      return { valid: false, reason: 'Clear the resource before building here.' };
    }
    if (this.decorations.some((decoration) => intersectsFoundation(decoration, DECORATION_FOOTPRINTS[decoration.type] ?? 0.45))) {
      return { valid: false, reason: 'Clear the ground detail before building here.' };
    }
    if (!blueprint.wallAttachment && this.units.some((unit) => !unit.dead && intersectsFoundation(unit, UNIT_TYPES[unit.type].radius + 0.18, buildingActorProfile(unit)))) {
  ...
  loadSnapshot(snapshot) {
    ...
    if(restored.wildlifeState && restored.wildlifeState.cadence!==initialWildlifeState(this.clock).cadence){this.wildlifeState.nextSpawnAt=initialWildlifeState(this.clock).nextSpawnAt;this.wildlifeState.cadence=initialWildlifeState(this.clock).cadence;}
  ...
  saveToStorage({ silent = false } = {}) {
    try {
      ...
      readSavedGameForBuildingUpgrade(localStorage);
  ...
  loadFromStorage() {
    try {
      if (typeof localStorage === 'undefined') return false;
      const snapshot = readSavedGameForBuildingUpgrade(localStorage);
      if (!snapshot) { this._announce('No Crownforge save is available in this browser.'); return false; }
      return this.loadSnapshot(snapshot);
    } catch {
      this._announce('The save could not be safely loaded. Your stored original has been kept.');
      return false;
    }
  }
