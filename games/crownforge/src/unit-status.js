import {deathlessActive,updateDeathlessHeart,deathlessCrossingDamage} from './deathless-heart.js?v=20260913-chorusfx3';
import { bearVariant } from './bear-variants.js?v=20260913-chorusfx3';
import {UNIT_TYPES} from './config.js?v=20260913-chorusfx3';
import {bearCrowdMultiplier,BEAR_FURY,bearFuryActive,bearEnrageActive,isFighter} from './bear-combat.js?v=20260913-chorusfx3';
...
export function unitStatuses(unit){
  ...
  if(unit.lastLightChorusTimer>0)statuses.push({id:'lastLightChorus',name:'Chorus of the Last Light',kind:'Team blessing',detail:`${Math.ceil(unit.lastLightChorusTimer)}s · 200% healing received`,summary:'One life spared becomes hope for the whole company.',lore:'When the Hearthkin’s shield kindles, its light passes from companion to companion. The old prayer asks no price of an enemy; it binds the living together until their strength returns.',effect:'Receives twice the normal healing for 60 seconds. Team heals restore 80 HP to tanks and 40 HP to other allies every two seconds while in healer range, capped at maximum health. Another ward refreshes the duration without stacking. No curse is applied to enemies.',rune:'ward'});
