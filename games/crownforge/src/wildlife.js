import {isWardProtected} from './unit-status.js?v=20260923-teamdock1';
import {encounterOpenness} from './combat-teams.js?v=20260923-teamdock1';
import { BEAR_VARIANT_IDS, bearVariant } from './bear-variants.js?v=20260923-teamdock1';
import { CONFIG, UNIT_TYPES } from './config.js?v=20260923-teamdock1';
...
function planGrizzly(sim,state,side=null,avoid=[],automatic=false) {
  ...
  // Search beyond the expanded starting clearing without relaxing canopy or route safety.
  const radii=[20,28,38,52,72,95,115,135,155,175,195,220], count=radii.length*32;
  let routes=0;
...
function hunt(sim,bear) {
  ...
  if(bear.threatTankId&&current?.id===bear.threatTankId){
    if(bear.command!=='attack'||bear.attackTarget!==current.id)sim._retaliateGrizzly(bear,current);
    return;
  }
