import {updateWizardPositioning} from './wizard-positioning.js?v=20260921-formationmilitia1';
import {updateStormDragons} from './storm-dragon.js?v=20260921-formationmilitia1';
...
export function castWizardSpell(sim,wizard,target){
 if(wizard.dead||target.dead||wizard.eventideVeil>0)return;
...
export function updateWizardMagic(sim,dt){
 ...
 sim.wizardProjectiles=(sim.wizardProjectiles??[]).filter(p=>{
  p.age+=dt;const target=sim.units.find(u=>u.id===p.targetId&&!u.dead),source=sim.units.find(u=>u.id===p.sourceId);if(!target||p.age>4)return false;
  const gap=dist(p,target),step=36*dt;
...
export {drawWizardMagic} from './starveil-effects.js?v=20260921-formationmilitia1';
