import {CONFIG,UNIT_TYPES,RESOURCE_SIZE_TIERS} from './config.js?v=20260921-formationmilitia1';
...
export function eventideDestination(sim,w,threat){
 ...
 for(const radius of [40,48,58,70,86])for(let i=0;i<32;i++){
  const a=axis+i*Math.PI/16,point={x:threat.x+Math.cos(a)*radius,z:threat.z+Math.sin(a)*radius};
  if(distance(w,point)<24||enemies.some(u=>distance(u,point)<EVENTIDE.escapeClearance)||!openWizardLanding(sim,w,point))continue;
  const score=distance(w,point)+radius*.12;
...
export function escapeEventide(sim,w,threat){
 ...
 (sim.wizardRifts??=[]).push({from,to:{...point},age:0});
 // Old projectiles and queued starbursts must not immediately reclaim aggro.
...
export function prepareWizardPosition(sim,w,target){
 ...
 const desired=Math.max(EVENTIDE.distance,...healers.map(u=>distance(u,target)+6));
 if(span>=desired-2&&distance(w,target)<=UNIT_TYPES.wizard.range&&hasLine){w.path=[];w.velocityX=w.velocityZ=0;return;}
