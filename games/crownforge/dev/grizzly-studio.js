import { GrizzlyRenderer } from '../src/grizzly-renderer.js?v=20260923-teamdock1';
import { BEAR_DEATH } from '../src/bear-combat.js?v=20260923-teamdock1';
import { GRIZZLY_MOTION, GRIZZLY_ATTACKS, grizzlyProjection } from '../src/grizzly-motion.js?v=20260923-teamdock1';
import { createCharacterRigs } from '../src/character-rigs.js?v=20260923-teamdock1';
...
function tick(t){
 ...
 for(let d=0;d<4;d++){
  ...
  if(bones.checked){ctx.strokeStyle='#ffcc62';ctx.beginPath();ctx.moveTo(x-size*.4,y);ctx.lineTo(x+size*.4,y);ctx.stroke();}
 ...
 status.textContent=`${bear.ready?'Artwork ready':'Loading artwork'} · ${action.selectedOptions[0].text} · ${Math.round(clock%duration()/duration()*100)}% · ${frames} frames · ${errors.length} errors`;
 window.grizzlyReview={frames,errors,ready:bear.ready,action:action.value};requestAnimationFrame(tick);
