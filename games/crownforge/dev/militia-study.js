import oldArt from '../src/painted-roster/militia.js';
import art from './militia-study-art.js';
import {PaintedRosterRenderer} from '../src/painted-roster/painted-roster-renderer.js';
import {CHARACTER_RIGS} from '../src/character-rigs.js?v=20260921-formationmilitia1';
...
pause.onclick=()=>{playing=!playing;pause.textContent=playing?'Pause':'Play';};document.querySelector('#step').onclick=()=>{playing=false;pause.textContent='Play';if(action==='attack'){const next=(attackFrame(time)+1)%8;time=durations.slice(0,next).reduce((a,b)=>a+b,0)+.001;}else if(action==='walk'){time=((Math.floor(time/1.3*8)+1)%8)*1.3/8+.001;}};
