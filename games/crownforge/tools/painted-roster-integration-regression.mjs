...
test('ten original fighters preserve their 240 authored action views alongside the wizard',()=>{
 ...
 assert.equal(views,240);assert.equal(frames,820);
 ...
 for(const type of PAINTED_FIGHTER_TYPES){const art=arts[type],definition=CHARACTER_RIGS[type];
  for(const [facing,view] of ['sw','se','ne','nw'].entries()){
   ...
   for(const phase of ['anticipation','contact','recovery']){
    ...
    const sample=paintedRosterFrame(unit,art,definition);assert.equal(sample.action,'attack');const authored=art[view].attack.phases?.[phase];assert.equal(sample.index,authored?authored[Math.floor(.7*authored.length)]:{anticipation:1,contact:2,recovery:3}[phase]);
   }
   ...
   unit.dead=true;unit.deathAge=100;assert.equal(paintedRosterFrame(unit,art,definition).index,art[view].death.frames.length-1);
  }
 }
});
