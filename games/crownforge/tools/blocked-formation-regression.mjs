...
test('Heart heals two percent on five-second pulses, saves pulse progress, and stops at expiry',()=>{
 ...
 const expiring={type:'grizzly',hp:100,maxHp:1000,lastStandTimer:1,lastStandSpent:true,lastStandHealElapsed:4};updateLastBastion(expiring,1);assert.equal(expiring.hp,120);assert(!lastBastionActive(expiring));updateLastBastion(expiring,10);assert.equal(expiring.hp,120);
