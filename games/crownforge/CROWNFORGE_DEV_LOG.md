...
## September 4, 2026 — Natural gait and attached weapon correction

Revisited the first Living Motion release after player feedback identified outward elbow bends, marching legs, poor back layering and a floating axe. Researched Animation Mentor, Adobe, Blender, Epic sockets and BioMotion Lab. Replaced locomotion/idle/carry joint solving with anatomical 3D motion projected into the four existing views; corrected opposing arm timing, low swing clearance, seam velocities, heel/toe rotation and actual-motion cadence. Added a calibrated palm attachment and hand-over-shaft drawing in every direction. No new raster generation or gameplay balance changes. Strengthened regressions to check true anatomical lengths rather than enforce the erroneous fixed screen-space lengths, along with ground contact and actual render attachment/order. See `CROWNFORGE_NATURAL_WALK_RELEASE.md` for sources, validation and scope.
...
Delivery: `5f3b040` serves live as `20260905-softelbow1`. Build and syntax checks passed; all 21 changed source/mirror files matched. Six deployed motion/registry/renderer/studio files matched the tested source byte for byte. Production field review showed the Crownwarden in real walking state; live studio review confirmed the four-view elbow bend and attached axe, with arm detail left playing for the user.

## September 5, 2026 — Crownwarden elbow surfaces and braid cleanup

Player close-up review exposed sleeve/forearm seams and a stray skin patch below the right-profile braid. These source-art defects remained despite the passing joint-motion checks. Preserved the existing gentle walking bend and all gameplay timing. Trimmed excess rigid skin from the sleeve crops while retaining the complete linen fringe, measured all four views' cuff/forearm attachment points, and placed the rolled cuff over the forearm insertion for Crownwarden Hearthkin only. The right braid's rectangular atlas cell included part of a neighbouring arm; a source polygon now excludes that component from every cached detail level without changing the braid's scale or pivot. Original PNG bytes remain unchanged.

Validation: surface regression 5/5, including real source alpha at attachments, painted joint coverage through all four walking views, preserved fine braid pixels, removal of all 1,377 significant neighbouring skin pixels, actual cache clipping, and unchanged arm stacking for the other 11 characters. Crown elbow regression 4/4; character loading 4/4; Hearthkin rig 23 actions/four views; full roster motion 12 rigs/544 directional states/44,064 poses/88,128 grips. All passed. Local browser close-ups checked walking, chopping, and building across all four views. Release marker: `20260905-cuffbraid3`.

Delivery: `927a662` is live as `20260905-cuffbraid3`. Build and changed-script syntax checks passed; all 21 changed source/production files matched. Seven deployed art/rig/registry/renderer/main/studio files matched the tested source byte for byte. Live four-view walking close-up confirmed the cleaned cuff joins and absence of the stray skin patch under the right braid. The Crownwarden studio remains open and playing with Arm detail enabled.

## September 5, 2026 — Crownwarden wrist proportions and seated profile shoulders

Player review exposed nearly 2:1 width steps between the forearms and small hands, plus moving profile sleeves below and ahead of the painted torso armholes. Re-measured the actual wrist creases, enlarged the hand surfaces independently for each view, narrowed the front/rear forearm surfaces, and removed distal crop overruns while preserving all original PNG bytes. All hands now use a measured 3.65-unit palm reach. The existing cuff overlap, soft walking elbow bend, and clean braid crop remain.

Added Crown-only profile surface fitting to measured torso sockets, using the same torso transform during upright, leaning and fallen poses. Free near-arm chains move as a connected assembly. Working hands keep their physical palm/tool targets; the shoulder depth and elbow are solved without stretching the 17/16-unit bones. Maximum attack extension permits less than .2 units of socket accommodation. Canonical motion, contact timing, game rules, and all other character surface fits remain unchanged. The studio's joint guide shows the fitted joints.

Validation: surface/wrist regression 7/7, profile-fit regression 4/4 across 7,452 poses, Crown elbow 4/4, loading 4/4, Hearthkin rig 23 actions/four views, and full roster 12 rigs/544 directional states/44,064 poses/88,128 grips. All passed. The new checks measure actual painted wrist widths and alignment, source socket placement, exact work palms, bone lengths, transition behavior, and unchanged shoulder rendering for the other 11 identities. Browser review covered walking close-ups and building/farming/attack poses, with complete-character and normal game-size views. Release marker: `20260905-armfit2`.

Delivery: `deaca20` is live as `20260905-armfit2`. Build and changed-script syntax checks passed; all 24 changed source/production files matched. Eight deployed artwork/fit/rig/registry/renderer/main/studio files matched the tested source byte for byte. Local carrying review retained connected hands and cargo. Live four-view walking review confirmed fuller hands, clean wrist joins, and the raised/rearward profile sleeve placement. The live Crownwarden Arm detail preview remains open and playing.

## September 5, 2026 — Crownwarden profile scale and wrist-edge refinement

Player follow-up identified side-view forearms/hands that still read too small and imperfect front/rear hand joins. Increased profile forearm width from 5.4 to 6.6 and hand width from 5.2 to 6.2, with a 4.4-unit profile palm link instead of 3.65. Both profile work arms now solve against the larger hands while keeping the existing physical palm/tool targets and 17/16-unit arm bones. Front/rear hand proportions and the seated profile shoulders remain.

Magnified source/assembly review found that each hand's extra proximal wrist skin overpainted the forearm with a hard rectangle, while horizontal forearm crop edges cut through the diagonal painted wrists. Removed the excess hand stump; leather cuffs have only two source pixels of overlap. Bare wrists use a narrow .5–.65-unit cached opacity transition to blend the differing source skin tones. Forearms retain enough source pixels to cover their diagonal edges and are clipped at the actual wrist plane, with .04-unit overlap. Original PNG bytes, canonical action timing, gameplay, and the other 11 characters remain unchanged.

Validation: surface regression 8/8, including clip-aware PNG edge sampling, 5% width/.1-unit front/rear edge tolerances, actual wrist-plane cuts, no distal overrun, and all 32 cached hand surfaces' opacity masks. Hearthkin rig 23 actions/four views; profile fitting 4/4 across 7,452 poses; Crown elbow 4/4; loading 4/4; full roster 12 rigs/544 directional states/44,064 poses/88,128 grips. All passed. Browser review used a temporary magnified eight-wrist inspection page, then the production studio's moving walking and building views. Release marker: `20260905-wristfit3`.
...
## September 5, 2026 — Crownwarden Hearthkin idle breathing

Added a restrained 3.6-second breathing loop to Crownwarden Hearthkin at ease, inspired by the base standing-animation reference in [WoW Dressing Room](https://www.wowhead.com/dressing-room). The chest and shoulders rise and fall, with a slight weight shift and delayed head, arm, cloth and braid movement. Each actor has a distinct phase. Both feet remain planted; all curves join continuously at the production clip boundary. Existing source artwork, physical limb lengths, measured sleeves/palms, walking, cargo, work and other characters retain their existing behavior. Only three motion/registry files change behavior; dependent imports and preview entry points receive cache version `20260905-idlebreath1`.

Validation: five focused idle tests cover visible amplitude, both planted feet in all four views, fitted limb lengths, loop position/velocity, individual phases, the zero-speed production animation clock, walk/idle transitions and unaffected action poses. Existing Crown elbow/fit/surface tests pass 16/16. Full-roster regression passes 12 rigs, 544 directional states, 44,064 poses and 88,128 painted grips. Both Crown and Ashen worker gameplay, military movement/combat, wards and save restoration pass. Browser review covers exhale/inhale extremes and walking/idle at character-detail and 28%/70% game sizes. Deployment verification follows the release push.

Release checks: the site build passed; all 56 changed scripts passed syntax checks, all 39 scoped source/public file pairs match, and the diff is whitespace-clean. Unrelated generated website pages and the pre-existing remediation mirror difference were restored after the build. The optional root website verification script is absent from this production branch; Crownforge-specific motion, gameplay and browser checks above are the release validation.

Delivery: `fb51e1df70c48997a466caff887c2cdced29e56d` is live as `20260905-idlebreath1`. All 36 changed runtime modules and pages match the tested local SHA-256 hashes (2026-09-06T00:51:53.212238+00:00). The published Crownwarden idle studio loaded all four authored views and is open, playing at normal speed, with the large/detail and 28%/70% game-size samples visible.

## September 5, 2026 — Player-selected Crownforge soundtrack

Added the four user-supplied original MP3 recordings: The Door Beneath the World, The Forgotten Stair, The Last Rune and Cavernous Wonder. Kept Lantern Under Stone as the fifth track. The default All songs option plays each once in order and then repeats the album. Choosing one title loops that recording, and the browser remembers the choice. A single audio element prevents overlapping music; mute pauses and unmute resumes. Match reset and save/load do not restart the music. The command deck now has an accessible soundtrack dropdown and current-song status. Cache marker: `20260905-playlist1`.

All four imported files match their Downloads originals byte for byte (17,137,474 bytes total); originals are untouched. MUSIC.md catalogs the Crownforge recordings, source/runtime storage arrangement and SHA-256 hashes. Other games and the home page retain their own audio folders; no central cross-project library was created.

Validation: music regression 6/6 covers three album cycles, every single-song selection, native loop flags, preferences, blocked storage, mute/switch/resume/reset and autoplay denial/retry. Browser review confirmed the dropdown, current title and mute control inside active gameplay. A local server with HTTP Range support decoded all five MP3s; seeking near each ending exercised real browser ended events through the full album and its return to track one. Native single-song playback looped The Last Rune twice. The earlier plain local HTTP server could play the tracks but could not seek to unbuffered endings; the Range-capable server resolved that test limitation. Test playback was paused after completion. Push and deployed-byte verification follow.

Release checks: site build passed; all 11 scoped source/public pairs match; changed scripts pass syntax checks and the diff is whitespace-clean. Unrelated root-site build outputs were restored. Production was refreshed before the release; no concurrent branch changes were present.

Delivery: feature commit `ba4c37d` is live as `20260905-playlist1`. All nine runtime/style/page/audio files match the tested source SHA-256 hashes (2026-09-06T01:19:20.052205+00:00). All five recordings also serve HTTP 206 byte ranges correctly. The published game presents All songs and all five individual titles in its soundtrack selector.

## September 5, 2026 — Construction and gathering smoothness

Addressed reported recurring freezes after building a Timber Yard and ordering three workers to chop. Profiling the full seed-42 woodland (2,520 opening entities, active enemy economy) identified repeated detailed hull calculations, repeated spatial-bucket assembly, exhaustive searches to every building approach, and global searches toward small enclosed drop-off pockets. Cached immutable hull edges/bounds/SAT projections; added exact broad-phase rejection, bounded navigation-version-aware spatial query caching, and radius-sized collision queries. Building approach selection now prefers clear/cached routes and stops at the first reachable detour in proximity/entrance order. A small reverse connectivity probe rejects proven enclosed pockets while retaining complete searches for larger connected areas. Route smoothing checks farther safe segments first. Placement reuses its preview for the cursor and stops checking access cells after finding the required two. All painted perimeters and exact collision geometry remain unchanged. Carry badges now read Wood, Food, Stone or Gold; quantities remain in unit details. Runtime cache marker: `20260905-smooth1`.

Same 80-second seeded CPU-profile scenario, before/after: valid foundation checks 164–191 ms → 2.5–5.6 ms; maximum drop-off route search 304 ms → 1.0 ms; simulation p99 63.9 ms → 2.4 ms; updates over 50 ms 17 → 1, with 120 wood delivered in both runs. Extended 350-second construction/gathering test completed the Timber Yard and delivered 672 wood; simulation p99 3.9 ms, maximum 77.1 ms. These timings are measurements on this machine, not device-independent frame guarantees.

Browser verification used the actual game, full artwork/forest, the input placement handler, construction, three workers, repeated tree work and deposits. Final placement selection took 2.9 ms and its click handler 6.7 ms. At 81 seconds: 4,856 simulation/render samples, simulation p99 0.7 ms, render p99 3.5 ms, no measured frame gaps over 100 ms, and the rendered cargo label was Wood. Continued observation and production byte verification follow the push.

Validation: 313,796 comparisons against the previous geometry matched exactly. New smoothness regression covers 50,610 geometry cases, 101 route selection scenarios, bounded/invalidation-safe blocker caching, fast rejection of a sealed destination, an obligatory long detour, and the complete 350-second economy run. Existing checks pass 50 foot/mounted building routes, 22 construction completions, 25 repairs, 12 recruitment exits, eight gate crossings, gathering, Livingwood/save restoration, Hearthkin economy/work/combat, meadow behavior, and all six soundtrack checks. The site build and syntax/diff checks pass. Scoped source/public files match; unrelated website build outputs and the pre-existing remediation mirror difference were restored. User saves, artwork and music assets are retained.


## September 5, 2026 — The Greatwood

Replaced sparse woodland placement with dense, habitat-coherent pine, spruce and broadleaf forests, irregular clearings and shared stand-height variation. Added four original RGBA conifer silhouettes and four rich meadow/forest-floor materials. Ground plants now follow lushness and flower drifts while preserving rooted wind/contact behavior. New worlds have roughly twice as many harvestable trees; existing saves retain positions, depletion, structures and clearings.

The denser forest exposed repeated whole-map searches to unreachable resources. Added a bounded resource-only connectivity map, shared across units and invalidated with navigation changes, to reject disconnected destinations before A*. Preserved precise collision, complete searches in connected regions, planned wall-clearing paths and gathering endpoint exceptions. Added mipmapped tree art, cached live resource render entries, and incremental forest-floor updates.

Greatwood, Livingwood, all 15 meadow, building navigation/services/defense, gathering, Hearthkin gameplay and dawn visual regressions pass. An actual pre-release save loads with resource/building state preserved exactly. The 350-second full simulation completed a Timber Yard and delivered 552 wood, with 9.0 ms p99 and 58.6 ms maximum updates. Browser review covers all woodland habitats and the full game; placement took 9.9 ms and workers built, chopped, carried Wood and delivered normally. Artwork, exact prompts, reference sources and validation are in GREATWOOD_RELEASE.md and GREATWOOD_ART_PROMPTS.json. Runtime marker: 20260905-greatwood3.
...
## September 5, 2026 — Woodland radiance and camera smoothness

Implemented three presentation upgrades: warm canopy lighting with painted-silhouette shadows and gentle rooted sway; less repetitive Retina-aware ground materials; and woodland light pools, mist and sunbeams. Added Dawn/Day/Dusk controls to the Greatwood preview. Original artwork, character motion, music, collision and saved coordinates remain intact.
...
## September 6, 2026 — Wildwood Watch

Enemy Hearthkin now choose reachable gathering jobs, recover from stalled/oscillating work, complete cargo deliveries and resume unfinished construction. Productive field work is retained. Idle Ashen military establish recurring perimeter patrols, respond to wildlife and Crown threats, yield to raids and resume afterward.
