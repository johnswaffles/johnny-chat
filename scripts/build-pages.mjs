  ...
  <script>
    (function () {
      ...
      function renderReader(post) {
        ...
        if (commentListEl) {
          const comments = Array.isArray(post.comments) ? post.comments.filter((comment) => !comment.hidden) : [];
          if (!comments.length) {
            commentListEl.hidden = true;
            commentListEl.innerHTML = "";
          } else {
                ...
                return (
                  '<details class="comment-card"' + (mobileOpen ? "" : " open") + '>' +
                    '<summary>' +
                      ...
                      '<div class="comment-card-meta">Posted by ' + escapeHTML(comment.author || "Anonymous") + ' on ' + escapeHTML(formatDate(comment.createdAt)) + '</div>' +
                    '</summary>' +
                    '<div class="detail-body">' + escapeHTML(comment.message).replace(/\\n/g, "<br>") + '</div>' +
                    '<div class="comment-card-mobile-note">Tap to collapse or expand this reply.</div>' +
                  '</details>'
                );
              }).join("");
          }
      ...
      async function deletePost(id) {
        ...
        statusEl.classList.remove("error");
        try {
          const response = await fetch(postsUrl + "/" + encodeURIComponent(id) + "/delete", {
            method: "POST",
            headers: authHeaders({ Accept: "application/json" })
          });
          const payload = await response.json().catch(() => ({}));
