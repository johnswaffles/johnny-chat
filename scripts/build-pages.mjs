  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .eyebrow-button:hover {
      transform: translateY(-1px);
      box-shadow: 0 12px 24px rgba(45, 111, 64, 0.08);
    }
    .eyebrow::before {
      content: "";
      width: 8px;
    ...
    .moderation-bulk[hidden] {
      display: none;
    }
    .moderation-bulk .bulk-status {
      margin-right: auto;
      color: var(--copy);
      font-size: 13px;
    }
    .moderation-bulk label {
      display: inline-flex;
      align-items: center;
      gap: 6px;
      font-size: 13px;
    ...
    .moderation-bulk input[type="checkbox"] {
      width: 16px;
      height: 16px;
      accent-color: #2d6f40;
    }
    ...
    .spotlight-card .spotlight-actions .button {
      min-height: 34px;
      padding: 0 12px;
      font-size: 13px;
    }
    ...
    .community-launcher {
      ...
      background: linear-gradient(180deg, rgba(255,255,255,0.96), rgba(244,248,242,0.95));
      color: var(--ink);
      font: inherit;
      font-weight: 800;
      box-shadow: 0 18px 36px rgba(17, 38, 22, 0.12);
      backdrop-filter: blur(12px);
      cursor: pointer;
    }
    .community-launcher-dot {
      width: 12px;
      height: 12px;
      border-radius: 999px;
      background: linear-gradient(180deg, #2d6f40, #4d8b5c);
      box-shadow: 0 0 0 6px rgba(45, 111, 64, 0.12);
      flex: 0 0 auto;
    }
    .community-widget-panel {
      ...
      box-shadow: 0 24px 48px rgba(17, 38, 22, 0.18);
      backdrop-filter: blur(16px) saturate(135%);
      overflow: hidden;
      display: grid;
      gap: 10px;
    }
    .community-widget-panel[hidden] {
      display: none;
    }
    ...
    .community-widget-header:active {
      cursor: grabbing;
    }
    ...
    .community-widget-title strong {
      font-family: "Outfit", Arial, sans-serif;
      font-size: 16px;
      letter-spacing: -0.03em;
      line-height: 1.1;
    }
    .community-widget-title span {
      color: var(--copy);
      font-size: 12px;
      line-height: 1.35;
    }
    .community-widget-controls {
      display: inline-flex;
      align-items: center;
      gap: 6px;
      flex: 0 0 auto;
    }
    .community-widget-mini {
      min-height: 32px;
      min-width: 32px;
      padding: 0 10px;
      border-radius: 999px;
    ...
    .community-widget-empty {
      ...
      padding: 12px;
      border-radius: 16px;
      border: 1px dashed rgba(45, 111, 64, 0.18);
      background: rgba(255,255,255,0.78);
      color: var(--copy);
      font-size: 13px;
      line-height: 1.55;
    }
    .community-widget-message {
      max-width: 88%;
      padding: 10px 12px;
      border-radius: 16px;
      border: 1px solid rgba(16, 32, 21, 0.08);
      ...
      line-height: 1.55;
      color: var(--ink);
      box-shadow: 0 8px 16px rgba(17, 38, 22, 0.05);
    }
    .community-widget-message.user {
      margin-left: auto;
      background: linear-gradient(180deg, rgba(233, 246, 239, 0.96), rgba(224, 240, 230, 0.94));
    ...
    .community-widget-input:focus {
      outline: none;
      border-color: rgba(45, 111, 64, 0.3);
      box-shadow: 0 0 0 4px rgba(45, 111, 64, 0.1);
    }
    .community-widget-actions {
      display: flex;
      align-items: center;
      justify-content: space-between;
      gap: 8px;
    ...
    .community-widget-status {
      color: var(--copy);
      font-size: 12px;
      line-height: 1.4;
      min-height: 1.4em;
    }
    .community-widget-status.error {
      color: #9b1c1c;
    }
    .topic-pages {
      margin-top: 14px;
...
${chatSiteNav("home")}
  <main class="page">
    <section class="intro">
      <div class="panel hero">
        <button class="eyebrow eyebrow-button" type="button" data-compose-jump>Anonymous conversation</button>
        ...
        <div class="hero-actions">
          <a class="button button-primary" href="#compose" data-compose-jump>Write a post</a>
  ...
  <div id="community-widget" class="community-widget">
  ...
  <script>
    (function () {
      ...
      const communityWidgetRoot = document.getElementById("community-widget");
      ...
      let moderationSelection = new Set();
      const supportedKey = "618chat_supported_items";
      const communityWidgetKey = "618chat_community_widget_v1";
      const communityHistoryKey = "618chat_community_history_v1";
      ...
      function readCommunityHistory() {
        try {
          const parsed = JSON.parse(window.localStorage.getItem(communityHistoryKey) || "[]");
      ...
      function readCommunityWidgetPlacement() {
        try {
          const parsed = JSON.parse(window.localStorage.getItem(communityWidgetKey) || "null");
        ...
        return {
          left: Math.max(12, window.innerWidth - 360),
          top: Math.max(12, window.innerHeight - 460)
        };
      }

      function saveCommunityWidgetPlacement(placement) {
        window.localStorage.setItem(communityWidgetKey, JSON.stringify({
          left: Math.max(12, Math.round(Number(placement?.left) || 0)),
          top: Math.max(12, Math.round(Number(placement?.top) || 0))
        }));
      }

      function getCommunityWidgetBounds() {
        if (!communityWidgetRoot) return { width: 340, height: 440 };
        const rect = communityPanel && !communityPanel.hidden
          ? communityWidgetRoot.getBoundingClientRect()
          : communityLauncher.getBoundingClientRect();
        return {
          width: Math.max(communityWidgetRoot.offsetWidth || rect.width || 340, 280),
          height: Math.max(rect.height || 64, communityPanel && !communityPanel.hidden ? 440 : 64)
        };
      }

      function placeCommunityWidget(left, top) {
        if (!communityWidgetRoot) return;
        const bounds = getCommunityWidgetBounds();
        const nextLeft = Math.min(Math.max(12, Number(left) || 0), Math.max(12, window.innerWidth - bounds.width - 12));
        const nextTop = Math.min(Math.max(12, Number(top) || 0), Math.max(12, window.innerHeight - bounds.height - 12));
        communityWidgetRoot.style.left = nextLeft + "px";
        communityWidgetRoot.style.top = nextTop + "px";
        communityWidgetRoot.style.right = "auto";
        communityWidgetRoot.style.bottom = "auto";
        saveCommunityWidgetPlacement({ left: nextLeft, top: nextTop });
      }

      function ensureCommunityWidgetVisible() {
        if (!communityWidgetRoot) return;
        const current = {
          left: parseFloat(communityWidgetRoot.style.left || "0") || 0,
          top: parseFloat(communityWidgetRoot.style.top || "0") || 0
        };
        const stored = readCommunityWidgetPlacement();
        placeCommunityWidget(current.left || stored.left, current.top || stored.top);
      }

      function appendCommunityMessage(role, text) {
      ...
      function setCommunityStatus(message, isError = false) {
        ...
        communityStatus.textContent = String(message || "");
      ...
      function resetCommunityWidget() {
        ...
        if (communityInput) {
          ...
          communityInput.style.height = "auto";
      ...
      async function sendCommunityMessage(text) {
        ...
        if (!message || !communityMessages) return;

        communityHistory.push({ role: "user", content: message });
        saveCommunityHistory();
        appendCommunityMessage("user", message);
        setCommunityStatus("Thinking...");
        if (communityInput) {
          communityInput.value = "";
          communityInput.style.height = "auto";
        }
        ...
        thinking.className = "community-widget-message assistant";
        thinking.textContent = "Thinking…";
        communityMessages.appendChild(thinking);
        communityMessages.scrollTop = communityMessages.scrollHeight;
        ...
        try {
          const response = await fetch(apiBase + "/api/chat", {
            ...
            body: JSON.stringify({
              input: message,
              history: communityHistory.slice(0, -1).slice(-10),
              profile: "community"
            })
          });
          ...
          saveCommunityHistory();
          setCommunityStatus(err.message || "Could not send the message.", true);
        }
      }

      function handleCommunityWidgetDragStart(event) {
        if (!communityWidgetRoot || !communityPanel || communityPanel.hidden) return;
        const pointer = event.touches ? event.touches[0] : event;
        if (!pointer) return;
        const rect = communityWidgetRoot.getBoundingClientRect();
        communityDragging = {
      ...
      function handleCommunityWidgetDragMove(event) {
        if (!communityDragging || !communityWidgetRoot) return;
        const pointer = event.touches ? event.touches[0] : event;
        if (!pointer) return;
        const bounds = getCommunityWidgetBounds();
        const nextLeft = pointer.clientX - communityDragging.offsetX;
        const nextTop = pointer.clientY - communityDragging.offsetY;
        const clampedLeft = Math.min(Math.max(12, nextLeft), Math.max(12, window.innerWidth - bounds.width - 12));
        const clampedTop = Math.min(Math.max(12, nextTop), Math.max(12, window.innerHeight - bounds.height - 12));
        communityWidgetRoot.style.left = clampedLeft + "px";
        communityWidgetRoot.style.top = clampedTop + "px";
        communityWidgetRoot.style.right = "auto";
        communityWidgetRoot.style.bottom = "auto";
      }

      function handleCommunityWidgetDragEnd() {
        if (!communityDragging || !communityWidgetRoot) return;
        const left = parseFloat(communityWidgetRoot.style.left || "0") || 0;
        const top = parseFloat(communityWidgetRoot.style.top || "0") || 0;
        saveCommunityWidgetPlacement({ left, top });
        communityDragging = null;
      }
      ...
      function ensureVisibleAfterSubmission(post) {
        if (!post) return;
        const filteredPosts = applyBoardFilters(adminMode ? posts : posts.filter((item) => !item.hidden));
        if (filteredPosts.some((item) => item.id === post.id)) return;
        searchQuery = "";
        if (postSearchInput) {
      ...
      function getSelectedPostFromUrl() {
        try {
      ...
      function syncSelectedPostUrl(id) {
        if (!window.history || typeof window.history.replaceState !== "function") return;
        try {
          const url = new URL(window.location.href);
          if (id) {
      ...
      function copyPostLink(postId) {
        ...
        try {
          ...
          statusEl.textContent = err?.message || "Could not copy the share link.";
          statusEl.classList.add("error");
        }
      }

      selectedId = getSelectedPostFromUrl();

      async function setPinnedState(id, pinned) {
        try {
          const key = String(id || "").trim();
          if (!adminMode || !adminToken || !key) return;
          const response = await fetch(itemsUrl + "/" + encodeURIComponent(key) + "/pin", {
            method: "POST",
            headers: authHeaders({
              Accept: "application/json",
              "Content-Type": "application/json"
          ...
          if (!response.ok || !payload.ok) {
            throw new Error(payload.error || payload.detail || "Could not update pin state.");
          }
          await loadPosts();
        } catch (err) {
          statusEl.textContent = err?.message || "Could not update pin state.";
          statusEl.classList.add("error");
        }
      }

      async function runModerationBulk(action, ids, confirmMessage) {
        const uniqueIds = Array.from(new Set((Array.isArray(ids) ? ids : []).map((value) => String(value || "").trim()).filter(Boolean)));
        if (!uniqueIds.length) {
          statusEl.textContent = "Select one or more items first.";
          statusEl.classList.remove("error");
          return;
        }
        if (confirmMessage && !window.confirm(confirmMessage)) return;

        statusEl.textContent = action === "pin"
          ? "Updating pins..."
          : action === "restore"
        ...
        try {
          for (const key of uniqueIds) {
            ...
            const payload = await response.json().catch(() => ({}));
            if (!response.ok || !payload.ok) {
              throw new Error(payload.error || payload.detail || ("Could not " + action + " one of the selected items."));
            }
          }
          moderationSelection = new Set();
          await loadPosts();
          statusEl.textContent = action === "pin"
          ...
          statusEl.textContent = err.message || ("Could not " + action + " the selected items.");
          statusEl.classList.add("error");
        }
      }
      ...
      function getSpotlightPosts(sourcePosts) {
      ...
      function deriveBoardStats(sourcePosts) {
        ...
        (Array.isArray(sourcePosts) ? sourcePosts : []).forEach((post) => {
          ...
          if (post.pinned) stats.pinnedCount += 1;
          const postFlags = Number(post.flags || 0) || 0;
      ...
      function renderReader(post) {
        ...
        const readerShareBtn = readerEl.querySelector('[data-reader-action="share"]');
        if (readerShareBtn) {
          readerShareBtn.addEventListener("click", (event) => {
            event.preventDefault();
            event.stopPropagation();
        ...
        if (readerPinBtn) {
          readerPinBtn.addEventListener("click", (event) => {
            event.preventDefault();
            event.stopPropagation();
            void setPinnedState(post.id, !postPinned);
          });
        }
      ...
      function renderSpotlight(sourcePosts) {
        if (!spotlightEl) return;
        const postsForSpotlight = getSpotlightPosts(sourcePosts);
        if (!postsForSpotlight.length) {
        ...
        spotlightEl.innerHTML =
          '<div class="spotlight-head">' +
            '<div>' +
              '<h4>Top this week</h4>' +
              '<p style="margin:6px 0 0;color:var(--copy);">A quick look at the posts getting the most support right now.</p>' +
            '</div>' +
            '<span class="status-badge">Community spotlight</span>' +
          '</div>' +
          '<div class="spotlight-grid">' +
            postsForSpotlight.map((post) => {
              const excerptText = String(formatDisplayedMessage(post.message) || "").replace(/\\n/g, " ");
              return (
                '<article class="spotlight-card">' +
                  ...
                  '<p>' + escapeHTML(excerptText.slice(0, 140)) + (excerptText.length > 140 ? "…" : "") + '</p>' +
                '</article>'
              );
            }).join("") +
          '</div>';

        spotlightEl.querySelectorAll("[data-spotlight-open]").forEach((button) => {
          button.addEventListener("click", (event) => {
            event.preventDefault();
            event.stopPropagation();
      ...
      function createPostCard(post) {
        ...
        card.className = "post-card" + (post.id === selectedId ? " active" : "") + (post.pinned ? " pinned" : "");
        ...
        if (shareBtnEl) {
          shareBtnEl.addEventListener("click", (event) => {
            event.preventDefault();
            event.stopPropagation();
            copyPostLink(post.id);
          });
      ...
      function renderModerationPanel() {
        ...
        moderationPanelEl.innerHTML =
          '<div class="moderation-head">' +
            ...
            '<div class="moderation-stats">' +
              ...
              '<span class="status-badge pin">Pinned ' + (stats.pinnedCount || 0) + '</span>' +
          ...
          '<div class="moderation-bulk">' +
            '<span class="bulk-status">Select items in the queue, then apply a bulk action.</span>' +
            '<label><input type="checkbox" data-moderation-bulk-select-all> Select all</label>' +
            '<button class="button button-secondary" type="button" data-moderation-bulk-action="pin">Pin selected</button>' +
            '<button class="button button-secondary" type="button" data-moderation-bulk-action="restore">Restore selected</button>' +
            '<button class="button button-danger" type="button" data-moderation-bulk-action="delete">Delete selected</button>' +
          '</div>' +
                ...
                '<article class="moderation-item">' +
                  ...
                  '</div>' +
                      ...
                      '<div class="moderation-actions">' +
                        ...
                        '<button class="button button-secondary" type="button" data-moderation-action="pin" data-item-id="' + escapeHTML(item.id) + '">' + (item.pinned ? "Unpin" : "Pin") + '</button>' +
        ...
        moderationPanelEl.querySelectorAll("[data-moderation-action]").forEach((button) => {
          button.addEventListener("click", (event) => {
            ...
            if (action === "open") {
              ...
              const target = findPostByItemId(itemId);
              if (target) void setPinnedState(target.id, !target.pinned);
        ...
        moderationPanelEl.querySelectorAll("[data-moderation-select]").forEach((checkbox) => {
          const itemId = checkbox.getAttribute("data-moderation-select");
          if (itemId) {
            checkbox.checked = moderationSelection.has(itemId);
          }
          checkbox.addEventListener("change", () => {
            const key = String(itemId || "").trim();
            if (!key) return;
            if (checkbox.checked) {
              moderationSelection.add(key);
            } else {
              moderationSelection.delete(key);
            }
          });
        });
        const bulkSelectAll = moderationPanelEl.querySelector("[data-moderation-bulk-select-all]");
        if (bulkSelectAll) {
          bulkSelectAll.checked = queue.length > 0 && queue.every((item) => moderationSelection.has(item.id));
          bulkSelectAll.addEventListener("change", () => {
            if (bulkSelectAll.checked) {
              queue.forEach((item) => moderationSelection.add(item.id));
            } else {
              moderationSelection = new Set();
            }
        ...
        moderationPanelEl.querySelectorAll("[data-moderation-bulk-action]").forEach((button) => {
          button.addEventListener("click", () => {
            const action = button.getAttribute("data-moderation-bulk-action");
            if (!action) return;
            const ids = queue.filter((item) => moderationSelection.has(item.id)).map((item) => item.id);
            const prefix = action === "pin" ? "Pin" : action === "restore" ? "Restore" : "Delete";
            void runModerationBulk(action, ids, prefix + " the selected items?");
          });
        });
      }
        ...
        fetch(endpoint, {
          ...
          .then(({ response, payload }) => {
            ...
            if (targetId) {
              ...
              selectedId = targetId;
              if (nextPost) {
                rememberSubmission(nextPost, "comment");
              }
            } else {
              const nextPost = payload.post;
              posts = [nextPost, ...posts.filter((post) => post.id !== nextPost.id)];
              selectedId = nextPost.id;
              rememberSubmission(nextPost, "post");
              ensureVisibleAfterSubmission(nextPost);
            }
            form.reset();
            clearReplyTarget();
            clearDraft();
            statusEl.textContent = targetId
              ? "Reply added to the conversation board."
              : (payload.post?.hidden
                  ? "Message received and sent for review."
                  : "Message added to the conversation board.");
            statusEl.classList.remove("error");
            render();
            messageInput.focus();
            window.setTimeout(() => {
      ...
      if (communityWidgetRoot) {
        const isCompactViewport = window.matchMedia && window.matchMedia("(max-width: 760px)").matches;
        if (isCompactViewport) {
          placeCommunityWidget(window.innerWidth - 12, 12);
          closeCommunityWidget();
        } else {
      ...
      updateCommunityVoiceButton();
      document.querySelectorAll("[data-compose-jump]").forEach((trigger) => {
      ...
      if (communityResetBtn) {
        communityResetBtn.addEventListener("click", resetCommunityWidget);
      }
      if (communityForm) {
        communityForm.addEventListener("submit", async (event) => {
          event.preventDefault();
          await sendCommunityMessage(String(communityInput?.value || ""));
        });
      }
      if (communityInput) {
        communityInput.addEventListener("input", () => {
          communityInput.style.height = "auto";
          communityInput.style.height = Math.min(communityInput.scrollHeight, 140) + "px";
        });
        communityInput.addEventListener("keydown", (event) => {
          if (event.key === "Enter" && !event.shiftKey) {
            event.preventDefault();
            if (communityForm && typeof communityForm.requestSubmit === "function") {
              communityForm.requestSubmit();
            } else if (communityForm) {
              communityForm.dispatchEvent(new Event("submit", { cancelable: true, bubbles: true }));
            }
          }
        });
      }
      if (communityDragHandle) {
        communityDragHandle.addEventListener("pointerdown", (event) => {
          if (event.target instanceof Element && event.target.closest("button")) return;
          handleCommunityWidgetDragStart(event);
        });
      }
      window.addEventListener("pointermove", handleCommunityWidgetDragMove);
      window.addEventListener("pointerup", handleCommunityWidgetDragEnd);
      window.addEventListener("pointercancel", handleCommunityWidgetDragEnd);
