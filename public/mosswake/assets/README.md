...
The optional `player`, `boss`, `enemy-*`, `tree-back`, `tree-mid`, `tree-front`, `projectile-*`, `fx-slash`, `fx-impact`, and `boss-fx` keys already have renderer hooks. No gameplay code needs to change when an approved replacement is swapped in; add a manifest entry and keep the same anchor and cell dimensions.
...
| Guardian telegraph and defeat FX | `effects/guardian-telegraph-fx-generated-v1.png` | 4 columns × 4 rows, 312×312 source cells, 16 frames | Painted volley fans, slam rings, dash lanes, exposure crack, phase-break shards, and defeat motes; selected by Guardian telegraph and boss particle hooks |
...
The first-section renderer is visually coherent and performance-safe. The player, Rowan, Mossling, Hollow Guardian, Guardian combat FX, Lantern-blade combat FX, water/shoreline family, and layered tree family now use generated, alpha-prepared assets; the procedural fallback remains available for every other living entity and effect. The remaining high-impact replacements are ranked in **GRAPHICS TO GENERATE NEXT (remaining)** below, led by outdoor foliage and the remaining environment families. The persistent production log lives in `MOSSWAKE_ASSET_UPGRADE_TRACKER.md`.
