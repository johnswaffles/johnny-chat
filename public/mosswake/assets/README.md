...
The first-section renderer is visually coherent and performance-safe. The player, Rowan, Mossling, and Hollow Guardian now use generated, alpha-prepared sprite sheets; the procedural fallback remains available for every other living entity. The remaining high-impact replacements are ranked in **GRAPHICS TO GENERATE NEXT (remaining)** below, led by combat FX, the Guardian arena kit, and the rest of the named NPC set. The persistent production log lives in `MOSSWAKE_ASSET_UPGRADE_TRACKER.md`.
