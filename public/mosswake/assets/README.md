...
8. **NPC presentation set — ROWAN + BRINDLE MOVEMENT COMPLETE** — `npcs/rowan-sheet-generated-v1.png` is integrated for Rowan’s idle, walk, map-work, talk, and reaction states, and `npcs/brindle-directional-walk-generated-v1.png` now covers Brindle’s four-direction route movement. Tansy and Lumen still use the named/activity family for their role loops; add dedicated sheets only when their directional or interaction review identifies a concrete mismatch. The existing Rowan sheet remains the visual reference to use for consistency.
