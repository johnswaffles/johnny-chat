...
The first-section renderer is visually coherent and performance-safe. The player, Rowan, and Mossling now use generated, alpha-prepared sprite sheets; the procedural fallback remains available for every other living entity. The remaining high-impact replacements are ranked in **GRAPHICS TO GENERATE NEXT (remaining)** below, led by the Hollow Guardian, combat FX, and the rest of the named NPC set. The older broad wishlist has been removed so this document does not imply that dozens of small assets are release blockers.
