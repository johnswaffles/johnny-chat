...
| assets/crownforge-roadside-props-v1.png | Fence section, weathered signpost, cargo nook, and settlement lantern used along authored roads | 1536 x 1024 RGBA, 2 x 2 source plate; nominal cell 768 x 512 | Four static prop cells; no direction or animation | Preserved 3:2 source-cell proportions; renderer draws authored size in world depth order | Timber, iron, crate, and amber-light accents use the same warm upper-left key as the settlement | Clean alpha; compact painted contact/ground treatment with no matte | **A-**, none | Original generated Crownforge roadside family; integrated as renderer-only prop cells | The small family is placed sparingly at entrances, junctions, and the main road edge; it does not participate in gameplay collision. |
| assets/crownforge-building-stages-v2.png | Legacy Crown Hall, Hearth House, and Waystore construction/completion art | 1254 x 1254 RGBA, 4 x 4 atlas; nominal cell 313.5 x 313.5 | Columns 0–2: legacy Crown Hall, Hearth House, Waystore. Rows 0–3: foundation, partial, near-complete, complete. Column 3 is unused. | Legacy square cells and retired building proportions | Earlier warm timber/straw/cool-stone family | Alpha-processed; painted grass/dirt bases and shadows retained | **C**, legacy fallback only | Original generated building family; matte removal with tools/prepare-world-atlases.mjs | Hearth House and Waystore are no longer seeded or player-buildable. New first-age buildings must receive dedicated approved art rather than extending this mixed-age sheet. |
| assets/crownforge-crown-hall-wood-v1.png | Active completed Crown Hall landmark | 1536 x 1024 RGBA, single transparent cutout | Static completed state; construction still uses the shared lifecycle atlas | Four-sided three-quarter landmark; runtime `renderSize: 1000`, exactly matching the `1000` Crown Barracks reference after the equal-scale pass | Primitive first-age timber, woven wattle, thatch, rope, blue-and-gold Crownwarden cloth, warm upper-left light | Transparent RGBA; grounded grass/dirt contact and no opaque edge matte | **A**, active | Original generated Crownforge landmark; inspected and registered as a versioned sibling rather than overwriting the prior hall | Replaces the stone-heavy starter silhouette and now reads as a reachable first-age civic building while preserving the existing south stairs, storage, training, selection, and placement systems. |
| assets/crownforge-barracks-first-age-v3.png | Active completed Crown Barracks landmark | 1536 x 1024 RGBA, single transparent cutout | Static completed state; construction uses the existing restrained progress reveal until a dedicated stage family is authored | Three-quarter timber longhouse and drill yard; runtime `renderSize: 1000`, 6 x 5 footprint; practice dummies provide the human-scale reference | Heavy carved timber, wattle, layered golden thatch, rope, packed earth, restrained blue-and-gold cloth, warm upper-left key | Generator checker matte removed with the established edge-connected alpha workflow; full silhouette and contact edge retained | **A-**, active; dedicated construction stages are the next building-art polish | Original ImageGen Crownforge asset using the approved timber Hall as the style reference and the retired stone Barracks only as a functional reference | Replaces `crownforge-barracks-v3.png` in live rendering. No stone walls, slate roof, castle tower, opaque plate, or checker background remains. |
| assets/crownforge-palisade-diagonal-right-v1.png, crownforge-palisade-diagonal-left-v1.png, crownforge-palisade-face-v1.png, crownforge-palisade-depth-v1.png | Active completed Palisade Wall directional family | Four 1536 x 1024 RGBA transparent cutouts | Four undirected screen-space views cover all eight snapped world directions: diagonal-right, diagonal-left, face-on horizontal, and receding depth | Upright stakes retain vertical screen posture while the wall footprint follows the projected direction; runtime repeats the chosen view at the existing 3-unit segment span | Rough warm timber, rope, packed earth, restrained blue-and-gold bindings, fixed upper-left light | Edge-connected and enclosed neutral matte removed; grass reads through every rail/post gap without a pale plate | **A**, active | Original ImageGen family referenced to the approved timber Hall and prior Palisade; cleaned with `prepare-crown-hall-v3.mjs` plus `prepare-neutral-matte.mjs` | Replaces runtime rotation of one diagonal picture. Opposite drag directions share the same undirected authored view, so no completed fence can turn sideways or upside down. |
...
| src/renderer.js | Road hierarchy, material, wear, and roadside detailing | World-space main road, village lanes, footpaths, plazas, marks, and static props | **A-** | Roads remain visual-only; the existing simulation navigation grid and collision rules are unchanged. |
...
| Roadside authored props | crownforge-roadside-props-v1.png + ROAD_DETAILS_ATLAS in src/config.js |
| Legacy Hall / Hearth House / Waystore lifecycle fallback | crownforge-building-stages-v2.png + BUILDING_STAGE_ATLAS |
| Completed Crown Hall landmark | crownforge-crown-hall-wood-v1.png + FIRST_AGE_ASSETS.townCenter |
| Completed Crown Barracks landmark | crownforge-barracks-first-age-v3.png + FIRST_AGE_ASSETS.barracks |
| Completed Palisade Wall | crownforge-palisade-diagonal-right-v1.png, crownforge-palisade-diagonal-left-v1.png, crownforge-palisade-face-v1.png, crownforge-palisade-depth-v1.png + resolveWallVisual in src/renderer.js |
