...
| assets/crownforge-crown-hall-wood-v1.png | Active completed Crown Hall landmark | 1536 x 1024 RGBA, single transparent cutout | Static completed state; construction still uses the shared lifecycle atlas | Four-sided three-quarter landmark; runtime `renderSize: 400`, about 0.4x the `1000` Crown Barracks reference after the compact-landmark pass | Primitive first-age timber, woven wattle, thatch, rope, blue-and-gold Crownwarden cloth, warm upper-left light | Transparent RGBA; grounded grass/dirt contact and no opaque edge matte | **A**, active | Original generated Crownforge landmark; inspected and registered as a versioned sibling rather than overwriting the prior hall | Replaces the stone-heavy starter silhouette and now reads as a reachable first-age civic building while preserving the existing south stairs, storage, training, selection, and placement systems. |
