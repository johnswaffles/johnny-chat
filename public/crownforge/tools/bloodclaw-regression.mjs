...
test('Bloodclaw deals fifty percent maximum health, can kill, and respects cooldown and wards',()=>{
 ...
 s._startAttackCycle(b,t);s._applyGrizzlyCleave(b,t);assert.equal(d.hp,d.maxHp*.5);assert.equal(h.hp,h.maxHp*.5);assert(b.animationEvents.some(e=>e.name==='bear_special_swipe'));
...
test('Aegis reduces all marked AoE by ninety percent, stacks with armor and Bastion, and cannot dodge',()=>{
 ...
 t.hp=t.maxHp*.1;const lastStand=s._applyUnitDamage(t,100,b,{areaOfEffect:true});assert(Math.abs(lastStand.damage-.4)<1e-8);
});
test('Bloodclaw hits primary tank and every direction once, ignores rage scaling, allies, wards and occlusion',()=>{
 ...
 assert(Math.abs(t.hp-t.maxHp*.99)<1e-8);
 for(const u of [front,rear,side])assert.equal(u.hp,u.maxHp*.5);
