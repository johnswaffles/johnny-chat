...
function checkSource(src){if(src.includes('/architecture-v2/')||src.includes('/architecture-first-age/')) return; assert(files.has(src.split('/').pop().split('?')[0]),src);}
for(const [type,art] of Object.entries(BUILDING_DEPTH)){checkSource(art.src);if(art.kind==='solid'&&!art.architectureV2)for(const stage of ['foundation','partial','nearComplete'])checkSource(BUILDING_COMPONENTS[type].stages[stage].src);}
