...
test('tank armor and evasion give exactly 50 landed enraged hits over 250 swings',()=>{
 ...
 for(let swing=1;swing<=250;swing++){const result=s._applyUnitDamage(tank,strikeDamage(bear,tank),bear);if(result.dodged)dodged++;else landed++;assert.equal(tank.dead,swing===250);}
 assert.equal(landed,50);assert.equal(dodged,200);assert.equal(tank.hp,0);assert.equal(UNIT_TYPES.grizzly.attack,58);
...
test('team healers pulse every nearby injured ally, with stronger tank healing and doubled range',()=>{
 const s=arena(),h=s.addUnit('villager',100,100,'player'),t=s.addUnit('shieldbearer',102,100,'player'),dps=s.addUnit('soldier',115,100,'player'),far=s.addUnit('soldier',117,100,'player'),enemy=s.addUnit('raider',103,100,'enemy');
 select(s,[h,t]);s.assignSelectedTeam();t.hp-=100;dps.hp=1;far.hp=1;enemy.hp=1;
 updateTeams(s,2);assert.equal(t.hp,t.maxHp-20);assert.equal(dps.hp,41);assert.equal(far.hp,1);assert.equal(enemy.hp,1);
 updateTeams(s,.1);assert.equal(t.hp,t.maxHp-20);
 updateTeams(s,2);assert.equal(t.hp,t.maxHp);assert.equal(dps.hp,81);
 h.stunTimer=2;t.hp-=100;updateTeams(s,2);assert.equal(t.hp,t.maxHp-100);
});
test('healers honor line of sight and stay dedicated to healing until removed',()=>{
 ...
 s._hasCombatLineOfSight=()=>true;h.command='build';updateTeams(s,2);assert.equal(t.hp,t.maxHp-120);assert.notEqual(h.command,'build');
 s._sendUnitToAttack(h,bear);assert.equal(h.attackTarget,null);updateTeams(s,2);assert.equal(t.hp,t.maxHp-40);
 select(s,[h]);s.leaveSelectedTeam();updateTeams(s,2);assert.equal(t.hp,t.maxHp-40);
});
