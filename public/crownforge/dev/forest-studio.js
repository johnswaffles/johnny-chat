...
function focus(name){view=name;renderer.resize();const p=targets[name],zoom=name==='kingdom'?Math.min(renderer.width/(CONFIG.mapWidth+CONFIG.mapHeight)/26*.93,renderer.height/(CONFIG.mapWidth+CONFIG.mapHeight)/13*.78):p.zoom;
...
canvas.addEventListener('wheel',e=>{e.preventDefault();const box=canvas.getBoundingClientRect();renderer.zoomAt(e.deltaY<0?1.12:1/1.12,{x:e.clientX-box.left,y:e.clientY-box.top})},{passive:false});
...
function frame(now){const delta=Math.min(.05,(now-previous)/1000);previous=now;ready=renderer.startupReadiness(simulation).ready&&renderer.meadow.sprites.length===12;
