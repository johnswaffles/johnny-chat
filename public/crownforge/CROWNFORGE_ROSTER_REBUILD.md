...
- 2026-09-05, Crownwarden-only walk refinement: added a small outward/backward elbow bow and forearm return toward the hip, following player feedback that front/rear arms looked locked straight. `relaxedWalkArms` is enabled only by the Crownwarden worker definition and only changes moving walk. Every other character/action family matches its pre-change pose hashes. Enlarged and player-size four-view review completed locally; validation and delivery recorded in `CROWNFORGE_DEV_LOG.md`. Do not apply this trial to the other characters without the user's next instruction.

- 2026-09-05, Crownwarden surface follow-up: corrected sleeve/forearm source crops, measured profile cuff attachments, and Crown-only cuff-over-forearm stacking after the new bend exposed poor joins. Removed neighbouring arm bleed from the right braid's cached source cell while retaining the original hair scale and pivot. No PNG or motion changes. Walking, chopping and building close-ups were reviewed; source-alpha and cached-clip checks supplement the earlier mathematical motion checks. Validation and delivery are recorded in `CROWNFORGE_DEV_LOG.md`, release `20260905-cuffbraid3`.
