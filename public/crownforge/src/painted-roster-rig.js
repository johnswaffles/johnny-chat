import {PaintedRosterRenderer,paintedRosterFrame,paintedRosterCacheStats} from './painted-roster/painted-roster-renderer.js?v=20260921-steadyhealers1';
...
function loadArt(type){
  if(!metadata.has(type))metadata.set(type,import(`./painted-roster/${type}.js?v=${type==='militia'?'20260921-toolbarheal1':type==='wizard'?'20260914-wizarddetail2':'20260909-cursedbears1'}`).then(module=>module.default));
...
export class PaintedRosterRig {
  ...
  prepare(view,action){
    ...
    const promise=this.painter.prepareAction(view,action,{resolution:['wizard','militia'].includes(this.definition.id)?'full':'small'}).then(ok=>{
