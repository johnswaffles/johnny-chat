import {drawFinalBenediction} from './final-benediction-vfx.js?v=20260923-teamdock1';
import {drawStormDragons} from './storm-dragon-renderer.js?v=20260923-teamdock1';
import {drawWizardMagic} from './wizard-magic.js?v=20260923-teamdock1';
import {drawLastLightChorus} from './last-light-chorus-vfx.js?v=20260923-teamdock1';
import { LivingCrownHall } from './crown-hall-living.js?v=20260923-teamdock1';
import {BlenderHearthkinRenderer} from './hearthkin-blender-renderer.js?v=20260923-teamdock1';
import {corpseLifetime} from './bear-combat.js?v=20260923-teamdock1';
import { ASHEN_HEARTHKIN_PAINTED_ART } from './ashen-hearthkin-painted-art.js?v=20260923-teamdock1';
import { PaintedHearthkinRenderer } from './hearthkin-painted-renderer.js?v=20260923-teamdock1';
import {curseRuneKind,drawCurseSigil,displayedUnitHealth} from './unit-status.js?v=20260923-teamdock1';
import { GrizzlyRenderer } from './grizzly-renderer.js?v=20260923-teamdock1';
import {paintedRosterFactories} from './painted-roster-rig.js?v=20260923-teamdock1';
import { CHARACTER_RIGS, createCharacterRigs } from './character-rigs.js?v=20260923-teamdock1';
import { BUILDING_DEPTH } from './building-depth-data.js?v=20260923-teamdock1';
import { BUILDING_COMPONENTS } from './building-components-data.js?v=20260923-teamdock1';
import { hasBuildingOutline, buildingPolygon } from './building-geometry.js?v=20260923-teamdock1';
import { drawHearthkinWard } from './hearthkin-rig.js?v=20260923-teamdock1';
import { CrownforgeLandscape } from './landscape.js?v=20260923-teamdock1';
import { ForestCache } from './forest-cache.js?v=20260923-teamdock1';
import { CrownforgeMeadow } from './meadow.js?v=20260923-teamdock1';
import { CrownforgeAtmosphere } from './atmosphere.js?v=20260923-teamdock1';
import { ANCIENT_FOREST_ATLAS, ASHEN_BUILDING_ASSETS, ASSET_RECTS, COMBAT_ATLASES, CONFIG, ENEMY_CAMP_ASSET, FACTION, GOLD_DEPOSIT_ASSETS, LARGE_STONE_ASSET, LIGHTING, RESOURCE_SIZE_TIERS, RESOURCE_TYPES, UNIT_TYPES, BUILDING_TYPES, VILLAGER_ATLASES, ENVIRONMENT_ATLAS, TREE_ATLAS, ROAD_DETAILS_ATLAS, BUILDING_STAGE_ATLAS, TREE_GROVE_ATLAS, WILDWOOD_FOREST_ATLAS, FIRST_AGE_ASSETS, resourceDepletionStage } from './config.js?v=20260923-teamdock1';
import { ANIMATION_EVENTS, animationDefinition, animationFrame, resolveAnimationState } from './animation.js?v=20260923-teamdock1';
...
export class CrownforgeRenderer {
  ...
  render(simulation, input, time) {
    ...
    drawWizardMagic(ctx,this,simulation,time);
    drawStormDragons(ctx,this,simulation,time);
  ...
  drawWorldEntities(ctx, simulation, time) {
    ...
    for (const entity of entities) {
      ...
      if (entity.dead && entity.deathAge >= corpseLifetime(entity)) continue;
  ...
  drawVillagerAsset(ctx, unit, screen, size, alpha = 1) {
    ...
    rig.reducedMotion=this.atmosphere.reducedMotion;
    ...
    for (const unit of simulation.units) {
      ...
      for (const event of unit.animationEvents) {
        ...
        if(event.name==='bear_special_swipe'){
          ...
          ctx.beginPath();for(let i=0;i<=96;i++){const p=ringPoint(i/96*TAU,14*(.82+progress*.18));if(i)ctx.lineTo(p.x,p.y);else ctx.moveTo(p.x,p.y);}
          ...
          for(let fan=0;fan<2;fan++)for(let claw=0;claw<3;claw++){
            const head=turn+fan*Math.PI,radius=11.6+claw*.95,arc=1.7;
            ctx.beginPath();
          ...
          for(let i=0;i<28;i++){const angle=turn+i*2.39996,p=ringPoint(angle,12+(i%5)*.5+progress*1.4,10+(i%4)*9);ctx.beginPath();ctx.arc(p.x,p.y,(1.4+i%3)*zoom,0,TAU);ctx.fillStyle=i%3?'#c90b31':'#ff8275';ctx.fill();}
          ctx.shadowBlur=0;
