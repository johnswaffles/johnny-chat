import { drawBlueWard, blueWardReadiness } from './hearthkin-blue-ward.js?v=20260923-firstage1';
import { HEARTHKIN_RIG_ART, HEARTHKIN_ARM_PARTS, HEARTHKIN_HAND_PARTS } from './hearthkin-rig-art.js?v=20260923-firstage1';
import { horseAssemblyTransforms } from './horse-assembly.js?v=20260923-firstage1';
import {fitCharacterSurfaces} from './character-surface-fit.js?v=20260923-firstage1';
import { hearthkinLocomotion, projectHearthkin } from './hearthkin-locomotion.js?v=20260923-firstage1';
import { anatomicalToolFrame, hearthkinWorkMotion } from './hearthkin-work-motion.js?v=20260923-firstage1';
import { drawCharacterEquipment, equipmentReadiness } from './character-equipment.js?v=20260923-firstage1';
import { drawCharacterShield, shieldGeometry, shieldReadiness } from './character-shields.js?v=20260923-firstage1';
...
export class HearthkinRig {
  ...
  part(key, index) {
    ...
    const levels = [256, 128, 64, 32].map(height => {
      ...
      const wristFade=source.wristFades?.[index];
      if(wristFade) {
        // Match the bare wrist materials over a narrow joint band instead
        // of exposing a hard color cut or an opaque rectangular skin stump.
        const mask=ctx.createLinearGradient(0,0,0,height*wristFade);
        ...
        ctx.fillRect(0,0,canvas.width,height);ctx.globalCompositeOperation='source-over';
      }
      return canvas;
    });
    ...
    const drawArm = left => {
      ...
      const lower=()=>{
        if(parts?.lower.clipAtWrist) {
          ...
          // wrist. Cut the final surface at the anatomical wrist plane,
          // rather than using a horizontal atlas edge that clips one side.
          const dx=c.x-b.x,dy=c.y-b.y,length=Math.max(.000001,Math.hypot(dx,dy)),ux=dx/length,uy=dy/length;
          const nx=uy,ny=-ux,at={x:c.x+ux*.04,y:c.y+uy*.04};
          ctx.save();ctx.beginPath();
          ctx.moveTo(at.x+nx*30,at.y+ny*30);ctx.lineTo(at.x-nx*30,at.y-ny*30);
          ctx.lineTo(at.x-nx*30-ux*100,at.y-ny*30-uy*100);ctx.lineTo(at.x+nx*30-ux*100,at.y+ny*30-uy*100);
          ctx.closePath();ctx.clip();
          attachedSegment(parts.lower,b,c,parts.lower.width??dimensions.forearm??5.4);ctx.restore();
