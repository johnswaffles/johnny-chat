import {SKYBREAKER_ART,SKYBREAKER_LORE} from './storm-dragon.js?v=20260914-wizardportrait5';
...
import {deathlessActive,updateDeathlessHeart,deathlessCrossingDamage} from './deathless-heart.js?v=20260914-wizardportrait5';
...
import {bearCrowdMultiplier,BEAR_FURY,bearFuryActive,bearEnrageActive,isFighter} from './bear-combat.js?v=20260914-wizardportrait5';
...
export function unitStatuses(unit){
  ...
  if(unit.skybreakerFavor>0)statuses.push({id:'skybreakerFavor',name:'Skybreaker’s Favor',kind:'Dragon blessing',detail:`${Math.ceil(unit.skybreakerFavor)}s · +50% Starshard damage`,summary:'A stolen star remembers the first thunder.',effect:'Starshard deals 54 damage instead of 36 for 12 seconds. Falling Constellation and Heavenrend damage are unchanged.',rune:'fury',art:SKYBREAKER_ART});
  if(unit.type==='wizard'){
  ...
  if(unit.type==='grizzly'){
   ...
   statuses.push({id:'falteringCrown',name:'Scent of the Faltering Crown',kind:'Predator’s instinct',detail:'Hunts the weakest nearby tank',summary:'The wild knows which oath is nearest breaking.',effect:'Always prioritizes the living, unwarded hostile tank with the lowest remaining health percentage within 24 units and clear line of sight. Reassesses as health changes; equal-health ties keep the current target. Other tanks cannot steal its attention by hitting it.',rune:'fury',art:bearVariant(unit).art});
  }
  ...
  if(bearEnrageActive(unit))statuses.push({id:'greatwoodColossus',name:'Greatwood Colossus',kind:'Enrage',detail:'50% true health · double size and damage',summary:'Wounded ancient blood awakens a towering guardian.',effect:'At half true health, doubles body size, collision radius, and damage for the rest of its life. Stacks with Wrath of the First Oath. Bloodclaw Reckoning sweeps within 14 units every 8 seconds: all hostile units take 50% maximum-health AoE damage before defenses.',rune:'fury',art:bearVariant(unit).art});
  if(bearEnrageActive(unit)||fury)statuses.push({id:'bloodclawReckoning',name:'Bloodclaw Reckoning',kind:'Area of effect',detail:'8-second cooldown · 50% maximum-health damage',summary:'Three crimson claws tear across the battle line.',effect:'Every hostile unit within 14 units and clear line of sight takes 50% of its maximum health as AoE damage before defenses. Hits in every direction, including tanks and healers. Ward protection is respected. Aegis of the Unbroken Crown reduces tank AoE damage by 90%, stacking with armor and Last Bastion.',rune:'fury',art:bearVariant(unit).art});
  ...
  if(unit.lastLightChorusTimer>0)statuses.push({id:'lastLightChorus',name:'Chorus of the Last Light',kind:'Team blessing',detail:`${Math.ceil(unit.lastLightChorusTimer)}s · 200% healing received`,summary:'One life spared becomes hope for the whole company.',lore:'When the Hearthkin’s shield kindles, its light passes from companion to companion. The old prayer asks no price of an enemy; it binds the living together until their strength returns.',effect:'Receives twice the normal healing for 60 seconds. Team heals restore 80 HP to tanks and 8 HP to other allies every two seconds while in healer range, capped at maximum health. Mercy of the Last Crown raises tank healing to 400 HP below 10% health. Another ward refreshes the duration without stacking. No curse is applied to enemies.',rune:'ward'});
  if(unit.lastLightWardTimer>0)statuses.push({id:'ward',name:'Last Light Ward',kind:'Protection',detail:`${Math.ceil(unit.lastLightWardTimer)}s · invulnerable`,summary:'Protected from damage and attack targeting.',lore:'At the edge of death, the Hearthkin’s last light becomes a refuge. For one minute, no blow can touch them.',effect:'Health restored. Untargetable by attacks until the ward expires. Also grants Chorus of the Last Light: the team receives double healing for 60 seconds. Bears seek other prey.',rune:'ward'});
  if(fury)statuses.push({id:'greatwoodFury',name:'Wrath of the First Oath',kind:'Fury · active',detail:'6× damage · frontal swipe',summary:'The ancient sentence answers the scent of death.',lore:'As a Greatwood bear’s strength falls to its last tenth, the first oath tightens. The condemned blood remembers the ruin of the star-grove. A final measure of the old gods’ wrath passes into every claw.',effect:'Active only at 10% true health or below. Multiplies damage by six and stacks with Greatwood Colossus for twelve times base damage. Landed hits remain lethal to non-tank fighters. Ordinary strikes require primary contact within the front 160-degree fan and clear line of sight. Bloodclaw Reckoning deals 50% maximum-health AoE damage before defenses to hostile units within 14 units in every direction. Bears prioritize the nearby tank with the lowest health percentage.',rune:'fury',art:bearVariant(unit).art});
