(() => {
  ...
  class Creature {
    constructor(kind, x, y, traits, rng) {
      ...
      this.energy = kind === "glowmite" ? 70 : 95;
      this.thirst = 0;
      this.age = 0;
  ...
  class Engine {
    ...
    seedLife() {
      for (let i = 0; i < 230; i++) this.paintPatch(this.rng.int(4, W - 5), this.rng.int(4, H - 5), "sunmoss", 3);
      for (let i = 0; i < 44; i++) this.addCreature("glowmite", this.rng.range(70, WORLD_W - 70), this.rng.range(70, WORLD_H - 70));
      for (let i = 0; i < 9; i++) this.addCreature("thornback", this.rng.range(70, WORLD_W - 70), this.rng.range(70, WORLD_H - 70));
    }
    ...
    addCreature(kind, x, y, traits) {
      ...
      const base = kind === "glowmite"
        ? { speed: 0.8, vision: 1, fertility: 1, hunger: 1, tint: this.rng.range(-0.8, 0.8) }
        : { speed: 0.72, stamina: 1, aggression: 1, sight: 1, metabolism: 1 };
    ...
    rollWeather() {
      ...
      if (this.weather === "drought") this.drought = clamp(this.drought + 0.35, 0, 1);
      if (this.weather === "rain" || this.weather === "storm") this.drought = clamp(this.drought - 0.22, 0, 1);
    }

    updateCells(dt) {
      const seasonGrowth = [1.32, 0.82, 1.08, 0.32][this.seasonIndex];
      const light = (Math.sin(this.tick * 0.006) + 1) * 0.5;
      const rain = this.weather === "rain" ? 0.011 : this.weather === "storm" ? 0.018 : -0.002 - this.drought * 0.006;
      for (let y = 0; y < H; y++) {
        for (let x = 0; x < W; x++) {
          const c = this.cell(x, y);
          if (c.type === "water") {
          ...
          if (c.moss > 0) {
            const growth = (light * 0.6 + c.water * 0.7 + c.nutrients * 0.4 - this.drought * 0.8) * 0.0022 * seasonGrowth;
            c.moss = clamp(c.moss + growth - (c.water < 0.09 ? 0.01 : 0), 0, 1);
            c.nutrients = clamp(c.nutrients - c.moss * 0.0009, 0, 1.2);
            if (c.moss > 0.58 && this.rng.chance(0.012 * seasonGrowth)) {
              ...
              if (n.type !== "water" && n.type !== "rock" && n.water > 0.16 && n.nutrients > 0.12) {
                n.moss = clamp(n.moss + 0.045, 0, 1);
              }
            }
          } else if (c.type !== "rock" && c.type !== "water" && c.water > 0.35 && c.nutrients > 0.55 && this.rng.chance(0.0012 * seasonGrowth)) {
            c.moss = 0.04;
          }
        }
      }
    ...
    updateGlowmite(g, dt) {
      ...
      g.cooldown -= dt;
      g.thirst += dt * 0.02;
      g.energy -= dt * (0.85 * g.traits.hunger + 0.18 * g.traits.speed);
      const predator = this.findNearest(g, "thornback", 95 * g.traits.vision);
      let tx = g.x + g.vx * 20;
      let ty = g.y + g.vy * 20;
      if (predator) {
        ...
        const moss = this.findBestMoss(g, 68 * g.traits.vision);
        if (moss) {
          tx = moss.x;
          ty = moss.y;
        }
      ...
      if (cell.moss > 0.04) {
        const bite = Math.min(cell.moss, 0.025);
        cell.moss -= bite;
        g.energy = clamp(g.energy + bite * 190, 0, 130);
      }
      if (cell.water > 0.4 || cell.type === "water") g.thirst = Math.max(0, g.thirst - dt * 0.22);
      if (g.energy > 92 && g.cooldown <= 0 && this.localCount(g.x, g.y, "glowmite", 45) < 7) {
        ...
        if (child) {
          child.energy = 46;
          g.energy -= 34;
          g.cooldown = 7 / g.traits.fertility;
        }
      }
      if (g.energy <= 0 || g.thirst > 1.45 || g.age > 150 + this.rng.range(0, 60)) g.dead = true;
    }
    ...
    updateThornback(t, dt) {
      ...
      t.cooldown -= dt;
      t.thirst += dt * 0.018;
      t.energy -= dt * (1.05 * t.traits.metabolism + 0.2 * t.traits.speed);
      const prey = this.findNearest(t, "glowmite", 125 * t.traits.sight);
      ...
      if (prey && dist2(t, prey) < 13 * 13) {
        prey.dead = true;
        prey.flash = 0.2;
        t.energy = clamp(t.energy + 58, 0, 160);
        t.flash = 0.18;
      }
      const cell = this.cellAt(t.x, t.y);
      if (cell.water > 0.4 || cell.type === "water") t.thirst = Math.max(0, t.thirst - dt * 0.18);
      if (t.energy > 118 && t.cooldown <= 0 && this.localCount(t.x, t.y, "thornback", 68) < 4) {
      ...
      if (t.energy <= 0 || t.thirst > 1.65 || t.age > 180 + this.rng.range(0, 80)) t.dead = true;
    }
    ...
    stats() {
      ...
      const stability = clamp(100 - Math.abs(45 - glow.length) * 0.9 - Math.abs(10 - thorn.length) * 2.6 + mossCells * 0.08 - this.drought * 18, 0, 100);
      return {
        mossCells, rotCells,
    ...
    disaster(type) {
      ...
      if (type === "plague") {
        for (const c of this.creatures) if (this.rng.chance(c.kind === "glowmite" ? 0.28 : 0.18)) c.dead = true;
      }
      if (type === "meteor") {
      ...
      if (type === "predators") {
        for (let i = 0; i < 8; i++) this.addCreature("thornback", this.rng.range(80, WORLD_W - 80), this.rng.range(80, WORLD_H - 80));
      }
    }

    rebalance() {
      const s = this.stats();
      if (s.mossCells < 80) for (let i = 0; i < 80; i++) this.paintPatch(this.rng.int(4, W - 5), this.rng.int(4, H - 5), "sunmoss", 2);
      if (s.glowmites < 12) for (let i = 0; i < 18; i++) this.addCreature("glowmite", this.rng.range(80, WORLD_W - 80), this.rng.range(80, WORLD_H - 80));
      if (s.thornbacks < 3 && s.glowmites > 20) for (let i = 0; i < 4; i++) this.addCreature("thornback", this.rng.range(80, WORLD_W - 80), this.rng.range(80, WORLD_H - 80));
      for (const c of this.cells) if (c.type !== "rock") { c.water = clamp(c.water + 0.12, 0, 1); c.nutrients = clamp(c.nutrients + 0.1, 0, 1.2); }
