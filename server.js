...
function getRealtimeTools(profile = "ai") {
  ...
  if (profile === "morrow") {
    ...
    tools.push({
      ...
      description: "Prepare or send an email to the Morrow user's own preconfigured private inbox. Natural requests like ‘send an email of my grocery list to myself’ and ‘email this to my inbox’ are email requests: call this tool, never say email is unavailable. Always call draft first. After drafting, offer one choice in one sentence: read it back or send it now. If the original request already says just send it or skip the readback, set delivery to send_now. While a draft is pending, no, nope, send it, and just send it mean send now; only cancel, don't send it, or never mind stop delivery. The destination is fixed by the app and can never be supplied or changed by the model.",
      parameters: {
        type: "object",
        properties: {
          action: {
...
- REQUIRED DEEP-THINKING ROUTING: Before giving a substantive answer, call think_deep if the request needs multiple calculations or steps, compares options under constraints, diagnoses a technical failure, reconciles conflicting evidence, makes a consequential recommendation, or explicitly asks you to think carefully or deeply. This is required even when you believe you can solve it yourself. For example, chatbot plan costs across different usage levels, surcharges, yearly totals, or break-even points MUST go to think_deep; do not calculate or answer those locally first. Only give one short acknowledgment before the tool call. The helper uses GPT-6 Astra with extra-high reasoning. Keep casual conversation, simple facts, and single-step questions in Realtime. Stay within your allowed scope. Pass the complete question, constraints, and only necessary context. After the tool returns, speak its answer naturally while preserving the conclusion, key reasons, uncertainty, and qualifications. Do not redo the analysis or call it again for the same request. This tool cannot take actions.
