...
async function callResponses({ input, max_output_tokens }) {
  const body = { model: CHAT_MODEL, input };
  ...
  try {
    ...
    if (/Unsupported parameter/i.test(msg) || /max_output_tokens/i.test(msg)) {
      const fallback = await openai.responses.create({ model: CHAT_MODEL, input });
      return fallback;
    }
    throw e;
  }
...
async function llm({ system, user, max_output_tokens = 2000 }) {
  const messages = [
    { role: "system", content: system },
    { role: "user", content: user }
  ];
  const resp = await callResponses({ input: messages, max_output_tokens });
...
async function chatHandler(req, res) {
  try {
    ...
    const systemPrompt = "You are Johnny, a pragmatic assistant. Answer clearly and directly. When you mention specific online facts, include plain URLs in the text.";
